...
class install_scripts_f2b(install_scripts):
    ...
    def update_scripts(self, dry_run=False):
        ...
        print(
            "Creating %s/fail2ban.service (from fail2ban.service.in): @BINDIR@ -> %s"
...
class install_command_f2b(install):
    user_options = install.user_options + [
        (
            "disable-2to3",
            None,
            "Specify to deactivate 2to3, e.g. if the install runs from fail2ban test-cases.",
        ),
...
if platform_system in ("linux", "solaris", "sunos") or platform_system.startswith(
    "gnu"
):
    data_files_extra.append(("/usr/share/doc/fail2ban", doc_files))
...
setup(
    ...
    data_files=[
        ("/etc/fail2ban", glob("config/*.conf")),
        ("/etc/fail2ban/filter.d", glob("config/filter.d/*.conf")),
        (
        ...
        (
            ...
            glob("config/action.d/*.conf") + glob("config/action.d/*.py"),
        ),
...
elements = {
    "/etc/": ["fail2ban.conf"],
    "/usr/bin/": ["fail2ban.py"],
    "/usr/lib/fail2ban/": ["version.py", "protocol.py"],
}
...
if obsoleteFiles:
    print("")
    print(
        "Obsolete files from previous Fail2Ban versions were found on " "your system."
    )
...
if isdir("/usr/lib/fail2ban"):
    print("")
    print(
        "Fail2ban is not installed under /usr/lib anymore. The new "
