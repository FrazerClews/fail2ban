...
class CommandActionTest(LogCaptureTestCase):
    ...
    def testSubstituteRecursiveTags(self):
        ...
        self.assertRaises(
            ValueError, lambda: substituteRecursiveTags({"A": "<B>", "B": "<A>"})
        )
        self.assertRaises(
            ValueError,
            ...
            self.assertEqual(
                substituteRecursiveTags(
                    OrderedDict(
                        (
                            (
                                "actionstart",
                                "ipset create <ipmset> hash:ip timeout <bantime> family <ipsetfamily>\n<iptables> -I <chain> <actiontype>",
                            ),
                            ("ipmset", "f2b-<name>"),
                            ("name", "any"),
                            ("bantime", "600"),
                            ...
                            (
                                "multiport",
                                "-p <protocol> -m multiport --dports <port> -m set --match-set <ipmset> src -j <blocktype>",
                            ),
                            ("protocol", "tcp"),
                            ("port", "ssh"),
                            (
                ...
                OrderedDict(
                    (
                        ...
                        (
                            "actiontype",
                            "-p tcp -m multiport --dports ssh -m set --match-set f2b-any src -j REJECT",
                        ),
                        (
                            "multiport",
                            "-p tcp -m multiport --dports ssh -m set --match-set f2b-any src -j REJECT",
        ...
        self.assertEqual(
            substituteRecursiveTags(
                {"A": "<<PREF>HOST>", "PREF": "IPV4", "IPV4HOST": "1.2.3.4"}
            ),
    ...
    def testSubstRec_DontTouchUnusedCallable(self):
        ...
        self.assertEqual(
            self.__action.replaceTag("test=<A>--<B>--<A>", cm), "test=0--<A><A>--0"
        )
        # should raise an exception (BOOM by replacement of tag "C"):
        self.assertRaises(
            ZeroDivisionError, lambda: self.__action.replaceTag("test=<C>", cm)
        )
    ...
    def testReplaceTag(self):
        ...
        self.assertEqual(
            self.__action.replaceTag("Text <HOST> text", aInfo), "Text 192.0.2.0 text"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.__action.replaceTag(
                "<matches>", {"matches": "some >char< should \\< be[ escap}ed&\n"}
            ),
            "some \\>char\\< should \\\\\\< be\\[ escap\\}ed\\&\\n",
        )
        self.assertEqual(
            self.__action.replaceTag(
                "<ipmatches>", {"ipmatches": "some >char< should \\< be[ escap}ed&\n"}
            ),
            "some \\>char\\< should \\\\\\< be\\[ escap\\}ed\\&\\n",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.__action.replaceTag(
                "09 <matches> 11", CallingMap(matches=lambda self: str(10))
            ),
            "09 10 11",
        )
    ...
    def testReplaceNoTag(self):
        ...
        self.assertEqual(
            self.__action.replaceTag("abc", CallingMap(matches=lambda self: int("a"))),
            "abc",
        )
    ...
    def testReplaceTagSelfRecursion(self):
        ...
        self.assertRaisesRegexp(
            ...
            lambda: self.__action.replaceTag(
                "<a><b>", self.__action._properties, conditional="family=inet4"
            ),
        )
    ...
    def testExecuteActionCheckRestoreEnvironment(self, tmp):
        ...
        self.assertLogged(
            "Invariant check failed", "Unable to restore environment", all=True
        )
    ...
    def testExecuteActionCheckOnBanFailure(self, tmp):
        ...
        self.__action.actioncheck = (
            "[ -e '%s' ] && echo 'check ok' || { echo 'check failed'; exit 1; }" % tmp
        )
        ...
        for i in (1, 2, 3):
            ...
            self.assertNotLogged(
                ...
                "stdout: %r"
                % ("repair ..." if self.__action.actionrepair else "started ..."),
                "stdout: %r" % "check ok",
                all=True,
            )
            # force error in ban:
    ...
    def testExecuteActionCheckRepairEnvironment(self, tmp):
        ...
        self.__action.ban({"ip": None})
        self.assertLogged(
            "Invariant check failed. Trying", "echo 'repair ...'", all=True
        )
        self.pruneLog()
        ...
        aInfo = CallingMap(
            {
                "ABC": "123",
                "ip": "192.0.2.1",
                "F-*": lambda self: {"fid": 111, "fport": 222, "user": "tester"},
            }
        )
        self.__action.actionban = (
            "echo '<ABC>, failure <F-ID> of <F-USER> -<F-TEST>- from <ip>:<F-PORT>'"
        )
    ...
    def testExecuteTimeout(self):
        ...
        self.assertTrue(
            time.time() >= stime + timeout and time.time() <= stime + timeout + 1
        )
    ...
    def testExecuteTimeoutWithNastyChildren(self):
        ...
        stime = time.time()
        self.assertFalse(
            CommandAction.executeCmd("bash %s" % tmpFilename, timeout=getnasty_tout)
        )
        # Wait up to 3 seconds, the child got killed
        cpid = getnastypid()
        # Verify that the process itself got killed
        self.assertTrue(
            Utils.wait_for(lambda: not pid_exists(cpid), 3)
    ...
    def testCallingMap(self):
        ...
        # Should work fine
        self.assertEqual(
            "%(callme)s okay %(dontcallme)s %(number)i" % mymap, "10 okay string 17"
        )
    ...
    def testCallingMapRep(self):
        m = CallingMap(
            {"a": lambda self: 2 + 3, "b": lambda self: self["a"] + 6, "c": ""}
        )
        s = repr(m)
        # only stored values (no calculated)
