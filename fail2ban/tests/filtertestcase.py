...
def _copy_lines_to_journal(
    in_, fields={}, n=None, skip=0, terminal_line=""
    ...
    def testAssertWrongTime(self):
        self.assertRaises(
            ...
            lambda: _assert_equal_entries(
                self, ("1.1.1.1", 1, 1421262060.0), ("1.1.1.1", 1, 1421262059.0), 1
            ),
        )
...
class IgnoreIP(LogCaptureTestCase):
    ...
    def testIgnoreSelfIP(self):
        ...
        for ip in ipList:
            ...
            self.assertNotLogged(
                "[%s] Ignore %s by %s" % (self.jail.name, ip, "ignoreself rule")
            )
    ...
    def testIgnoreIPNOK(self):
        ...
        if not unittest.F2B.no_network:  # pragma: no cover
            self.assertLogged(
                ...
                "Unable to find a corresponding IP address for 192.168.0.",
                all=True,
            )
    ...
    def testWrongIPMask(self):
        ...
        self.assertRaises(
            ValueError, self.filter.addIgnoreIP, "192.168.1.0/255.255.0.128"
        )
    ...
    def testWrongTimeZone(self):
        try:
            ...
            # real processing (all messages are new)
            ...
            for i in (1, 2, 3):
                self.filter.processLineAndAdd(
                    "2019-10-27 02:00:00 fail from 192.0.2.15"
                )
                # +3 = 3
            self.assertLogged(
            ...
            for i in (1, 2, 3):
                self.filter.processLineAndAdd(
                    "2019-10-27 04:00:00 GMT fail from 192.0.2.16"
                )
                # +3 = 6
            self.assertLogged(
                "192.0.2.16:1",
            ...
            #
            self.pruneLog(
                "[phase 3] other timestamp (don't match datepattern), regex matches"
            )
            for i in range(3):
                self.filter.processLineAndAdd(
                    "27.10.2019 04:00:00 fail from 192.0.2.17"
                )
                # +3 = 9
            self.assertLogged(
    ...
    def testAddAttempt(self):
        ...
        for i in range(1, 1 + 3):
            ...
            self.assertLogged(
                "Attempt 192.0.2.1", "192.0.2.1:%d" % i, all=True, wait=True
            )
        self.jail.actions._Actions__checkBan()
        self.assertLogged("Ban 192.0.2.1", wait=True)

    def testIgnoreCommand(self):
        self.filter.ignoreCommand = (
            sys.executable + " " + os.path.join(TEST_FILES_DIR, "ignorecommand.py <ip>")
        )
        self.assertTrue(self.filter.inIgnoreIPList("10.0.0.1"))
        self.assertFalse(self.filter.inIgnoreIPList("10.0.0.0"))
        self.assertLogged(
            "returned successfully 0", "returned successfully 1", all=True
        )
        self.pruneLog()
    ...
    def testIgnoreCommandForTicket(self):
        ...
        self.filter.ignoreCommand = (
            'if [ "<ip-host>" = "test-host" ]; then exit 0; fi; exit 1'
        )
        self.pruneLog()
        ...
        self.filter.ignoreCommand = (
            'if [ "<F-USER>" = "tester" ]; then exit 0; fi; exit 1'
        )
        self.pruneLog()
        self.assertTrue(
            self.filter.inIgnoreIPList(FailTicket("tester", data={"user": "tester"}))
        )
        self.assertLogged("returned successfully 0")
        self.pruneLog()
        self.assertFalse(
            self.filter.inIgnoreIPList(FailTicket("root", data={"user": "root"}))
        )
    ...
    def testIgnoreCache(self):
        ...
        self.filter.ignoreCommand = (
            'if [ "<ip>" = "10.0.0.1" ]; then exit 0; fi; exit 1'
        )
        for i in range(5):
            self.pruneLog()
            self.assertTrue(self.filter.inIgnoreIPList("10.0.0.1"))
        ...
        for i in range(5):
            ...
            self.assertTrue(
                self.filter.inIgnoreIPList(
                    FailTicket("tester", data={"user": "tester"})
                )
            )
            self.assertFalse(
                self.filter.inIgnoreIPList(FailTicket("root", data={"user": "root"}))
    ...
    def testIgnoreCauseOK(self):
        ...
        for ignore_source in ["dns", "ip", "command"]:
            ...
            self.assertLogged(
                "[%s] Ignore %s by %s" % (self.jail.name, ip, ignore_source)
            )

    def testIgnoreCauseNOK(self):
        self.filter.logIgnoreIp("example.com", False, ignore_source="NOT_LOGGED")
        self.assertNotLogged(
            "[%s] Ignore %s by %s" % (self.jail.name, "example.com", "NOT_LOGGED")
        )
...
class IgnoreIPDNS(LogCaptureTestCase):
    ...
    def testIgnoreCmdApacheFakegooglebot(self):
        ...
        cmd = os.path.join(
            STOCK_CONF_DIR, "filter.d/ignorecommands/apache-fakegooglebot"
        )
        ...
        for ip in bot_ips:
            self.assertTrue(
                mod.is_googlebot(mod.process_args([cmd, str(ip)])),
                "test of googlebot ip %s failed" % ip,
            )
        self.assertRaises(ValueError, lambda: mod.is_googlebot(mod.process_args([cmd])))
        self.assertRaises(
            ValueError, lambda: mod.is_googlebot(mod.process_args([cmd, "192.0"]))
        )
        ## via command:
        self.filter.ignoreCommand = cmd + " <ip>"
        for ip in bot_ips:
            self.assertTrue(
                self.filter.inIgnoreIPList(str(ip)),
                "test of googlebot ip %s failed" % ip,
            )
            self.assertLogged("-- returned successfully")
            self.pruneLog()
...
class LogFileMonitor(LogCaptureTestCase):
    ...
    def isModified(self, delay=2):
        ...
        return Utils.wait_for(
            lambda: self.filter.isModified(self.name), _maxWaitTime(delay)
        )

    def notModified(self, delay=2):
        """Wait up to `delay` sec as long as it was not modified"""
        return Utils.wait_for(
            lambda: not self.filter.isModified(self.name), _maxWaitTime(delay)
        )
    ...
    def testNewChangeViaGetFailures_rewrite(self):
        ...
        self.file = _copy_lines_between_files(
            GetFailures.FILENAME_01, self.name, skip=12, n=3, mode="w"
        )
        self.filter.getFailures(self.name)
        # self.assertRaises(FailManagerEmpty, self.filter.failManager.toBan)
        _assert_correct_last_attempt(self, self.filter, GetFailures.FAILURES_01)
    ...
    def testNewChangeViaGetFailures_move(self):
        ...
        self.file.close()
        self.file = _copy_lines_between_files(
            GetFailures.FILENAME_01, self.name, n=14, mode="w"
        )
        self.filter.getFailures(self.name)
        self.assertRaises(FailManagerEmpty, self.filter.failManager.toBan)
        ...
        _copy_lines_between_files(
            GetFailures.FILENAME_01, self.name, skip=14, n=1
        ).close()
        self.filter.getFailures(self.name)
        _assert_correct_last_attempt(self, self.filter, GetFailures.FAILURES_01)
...
class CommonMonitorTestCase(unittest.TestCase):
    ...
    def waitFailTotal(self, count, delay=1):
        ...
        ret = Utils.wait_for(
            lambda: self.filter.failManager.getFailTotal() >= (self._failTotal + count)
            and self.jail.isFilled(),
            _maxWaitTime(delay),
        )
    ...
    def waitForTicks(self, ticks, delay=2):
        ...
        return Utils.wait_for(
            lambda: self.filter.ticks >= last_ticks + ticks, _maxWaitTime(delay)
        )
...
def get_monitor_failures_testcase(Filter_):
    ...
    class MonitorFailures(CommonMonitorTestCase):
        ...
        def _wait4failures(self, count=2):
            ...
            self.assertTrue(
                self.isEmpty(_maxWaitTime(5)),
                "Queue must be empty but it is not: %s."
                % (", ".join([str(x) for x in self.jail.queue])),
            )
            self.assertRaises(FailManagerEmpty, self.filter.failManager.toBan)
            Utils.wait_for(
                lambda: self.filter.failManager.getFailTotal() >= count,
                _maxWaitTime(10),
            )
        ...
        def test_move_file(self):
            ...
            _copy_lines_between_files(
                GetFailures.FILENAME_01, self.name, skip=12, n=3
            ).close()
            self.assert_correct_last_attempt(GetFailures.FAILURES_01)
        ...
        def test_del_file(self):
            ...
            if hasattr(self.filter, "getPendingPaths"):
                self.assertTrue(
                    Utils.wait_for(
                        lambda: self.name in self.filter.getPendingPaths(),
                        _maxWaitTime(10),
                    )
                )
        ...
        def test_move_dir(self, tmp):
            ...
            self.file = _copy_lines_between_files(
                GetFailures.FILENAME_01, self.name, skip=12, n=1, mode="w"
            )
            self.file.close()
        ...
        def _test_move_into_file(self, interim_kill=False):
            ...
            _copy_lines_between_files(
                GetFailures.FILENAME_01, self.name + ".new", skip=12, n=3
            ).close()
        ...
        def test_delLogPath(self):
            ...
            self.assert_correct_last_attempt(
                GetFailures.FAILURES_01, count=6
            )  # was needed if we write twice above
    ...
    class MonitorJournalFailures(CommonMonitorTestCase):
        ...
        def _getRuntimeJournal(self):
            ...
            raise unittest.SkipTest(
                "systemd journal seems to be not available (e. g. no rights to read)"
            )
        ...
        def assert_correct_ban(self, test_ip, test_attempts):
            self.assertTrue(
                self.waitFailTotal(test_attempts, 10)
            )  # give Filter a chance to react
        ...
        def test_grow_file_with_db(self):
            def _gen_falure(ip):
                ...
                fields.update(TEST_JOURNAL_FIELDS)
                journal.send(
                    MESSAGE="error: PAM: Authentication failure for test from " + ip,
                    **fields
                )
                self.waitForTicks(1)
            ...
            self.jail.database.updateJournal(
                self.jail, "systemd-journal", MyTime.time(), "TEST"
            )
        ...
        def test_WrongChar(self):
            ...
            for l in (
                ...
                "error: PAM: Authentication failure for \xe4\xf6\xfc\xdf from 192.0.2.1",
                b"error: PAM: Authentication failure for \xe4\xf6\xfc\xdf from 192.0.2.1".decode(
                    "utf-8", "replace"
                ),
                "error: PAM: Authentication failure for \xc3\xa4\xc3\xb6\xc3\xbc\xc3\x9f from 192.0.2.2",
                "error: PAM: Authentication failure for \xc3\xa4\xc3\xb6\xc3\xbc\xc3\x9f from 192.0.2.2",
                b"error: PAM: Authentication failure for \xc3\xa4\xc3\xb6\xc3\xbc\xc3\x9f from 192.0.2.2".decode(
                    "utf-8", "replace"
                ),
            ):
                fields = self.journal_fields
...
class GetFailures(LogCaptureTestCase):
            ...
            for enc in (None, "utf-8", "ascii"):
                if enc is not None:
                    ...
                    if DefLogSys.getEffectiveLevel() > 7:
                        ...
                        # ensure decode_line logs always
                    self.filter.checkFindTime = False
    ...
    def testGetFailuresMultiLine(self):
        output = [("192.0.43.10", 2, 1124013599.0), ("192.0.43.11", 1, 1124013598.0)]
        self.filter.addLogPath(GetFailures.FILENAME_MULTILINE, autoSeek=False)
        self.filter.setMaxLines(100)
        self.filter.addFailRegex(
            r"^.*rsyncd\[(?P<pid>\d+)\]: connect from .+ \(<HOST>\)$<SKIPLINES>^.+ rsyncd\[(?P=pid)\]: rsync error: .*$"
        )
        self.filter.setMaxRetry(1)
        ...
        while True:
            try:
                foundList.append(_ticket_tuple(self.filter.failManager.toBan())[0:3])
            except FailManagerEmpty:
                break
    ...
    def testGetFailuresMultiLineMultiRegex(self):
        ...
        self.filter.addFailRegex(
            r"^.* sendmail\[.*, msgid=<(?P<msgid>[^>]+).*relay=\[<HOST>\].*$<SKIPLINES>^.+ spamd: result: Y \d+ .*,mid=<(?P=msgid)>(,bayes=[.\d]+)?(,autolearn=\S+)?\s*$"
        )
        self.filter.setMaxRetry(1)
...
class DNSUtilsNetworkTests(unittest.TestCase):
    ...
    def testUseDns(self):
        ...
        self.assertSortedEqual(
            res, ["93.184.216.34", "2606:2800:220:1:248:1893:25c8:1946"]
        )
    ...
    def testIPAddr_Equal6(self):
        self.assertEqual(
            IPAddr("2606:2800:220:1:248:1893::"), IPAddr("2606:2800:220:1:248:1893:0:0")
        )
    ...
    def testIPAddr_InInet(self):
        ...
        self.assertFalse(
            IPAddr("2606:2800:220:1:248:1893:25c8:1", IPAddr.CIDR_RAW).isInNet(ip6net)
        )
        # invalid not in net:
    ...
    def testIPAddr_CIDR(self):
        ...
        self.assertEqual(
            str(IPAddr("2606:28ff:220:1:248:1893:25c8::", 25)), "2606:2880::/25"
        )
        self.assertEqual(
            str(IPAddr("2606:28ff:220:1:248:1893:25c8::/ffff:ff80::")), "2606:2880::/25"
        )
        self.assertEqual(
            str(
                IPAddr(
                    "2606:28ff:220:1:248:1893:25c8::/ffff:ffff:ffff:ffff:ffff:ffff:ffff::"
                )
            ),
        ...
        self.assertEqual(
            str(
                IPAddr(
                    "2606:28ff:220:1:248:1893:25c8::/ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff"
                )
            ),
            "2606:28ff:220:1:248:1893:25c8:0",
        )
    ...
    def testIPAddr_CIDR_Wrong(self):
        ...
        self.assertRaises(
            ValueError, IPAddr, "2606:28ff:220:1:248:1893:25c8::/ffff::/::1"
        )
