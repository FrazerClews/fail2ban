...
import tempfile
import unittest
from ..client.configreader import (
    ConfigReader,
...
option = %s
    ...
    def testConvert(self):
        ...
        opts = self.c.getOptions(
            "Definition", (("int", "a", 0), ("bool", "b", 0), ("int", "c", 0))
        )
        self.assertSortedEqual(opts, {"a": 1, "b": True, "c": 0})
        opts = self.c.getOptions(
            "Definition", (("int", "a"), ("bool", "b"), ("int", "c"))
        )
        self.assertSortedEqual(opts, {"a": 1, "b": True, "c": None})
        opts = self.c.getOptions(
            "Definition", {"a": ("int", 0), "b": ("bool", 0), "c": ("int", 0)}
        )
        self.assertSortedEqual(opts, {"a": 1, "b": True, "c": 0})
    ...
    def testInaccessibleFile(self):
        ...
        if not os.access(f, os.R_OK):
            ...
            raise unittest.SkipTest(
                "Skipping on %s -- access rights are not enforced" % platform.platform()
            )
...
""",
        ...
        self.assertEqual(
            self.c.get("section", "e"), "5${b}"
        )  # no extended interpolation
        self.assertEqual(self.c.get("section", "z"), "section")  # __name__ works
        self.assertEqual(
            self.c.get("section", "zz"), "thesection"
        )  # __name__ works even 'delayed'
        self.assertEqual(
            self.c.get("section2", "z"), "3section2"
        )  # and differs per section ;)
...
""",
        ...
        self.assertEqual(
            self.c.get("jail", "b"), "jail-b-test-b-def-b,a:`jail-a-test-a-def-a`"
        )
        self.assertEqual(
            self.c.get("jail", "c"),
            'def-c,b:"jail-b-test-b-def-b,a:`jail-a-test-a-def-a`"',
        )
        self.assertEqual(
            self.c.get("jail", "d"), 'def-d-b:"def-b,a:`jail-a-test-a-def-a`"'
        )
        self.assertEqual(
            self.c.get("test", "c"), 'def-c,b:"test-b-def-b,a:`test-a-def-a`"'
        )
...
class JailReaderTest(LogCaptureTestCase):
    ...
    def testIncorrectJail(self):
        jail = JailReader(
            "XXXABSENTXXX", basedir=CONFIG_DIR, share_config=CONFIG_DIR_SHARE_CFG
        )
    ...
    def testJailActionFilterMissing(self):
        ...
        self.assertTrue(jail.isEnabled())
        self.assertLogged(
            "Found no accessible config files for 'filter.d/catchallthebadies' under %s"
            % IMPERFECT_CONFIG
        )
    ...
    def testStockSSHJail(self):
        ...
        jail = JailReader(
            "sshd", basedir=CONFIG_DIR, share_config=CONFIG_DIR_SHARE_CFG
    ...
    def testOverrideFilterOptInJail(self):
        unittest.F2B.SkipIfCfgMissing(stock=True)
        # expected include of common.conf
        jail = JailReader(
                ...
                [
                    ...
                    [
                        ...
                        [
                            [
                                "actionban",
                                'echo "name: ban, ban: <ip>, logs: a.log\nb.log\nc.log"',
                            ],
                            ["actname", "ban"],
                            ["name", "multi-log"],
                        ],
                ...
                [
                    ...
                    [
                        ...
                        [
                            [
                                "actionban",
                                'echo "name: log, ban: <ip>, logs: a.log\nb.log\nc.log\nd.log"',
                            ],
                            ["actname", "log"],
                            ["logpath", "a.log\nb.log\nc.log\nd.log"],
                            ["name", "multi-log"],
                ...
                [
                    ...
                    [
                        ...
                        [
                            [
                                "actionban",
                                'echo "name: test, ban: <ip>, logs: a.log\nb.log\nc.log"',
                            ],
                            ["actname", "test"],
                            ["name", "multi-log"],
                        ],
    ...
    def testVersionAgent(self):
        ...
        self.assertEqual(
            act[1], ["set", "blocklisttest", "action", "badips", "agent", useragent]
        )
...
class FilterReaderTest(LogCaptureTestCase):
    ...
    def testConvertOptions(self):
        ...
        # wrong int value 'X' for 'maxlines'
    ...
    def testFilterReaderSubstitionSet(self):
        output = [
            ["set", "jailname", "addfailregex", "to=sour@example.com fromip=<IP>"]
        ]
        filterReader = FilterReader(
            "substition",
            "jailname",
    ...
    def testFilterReaderExplicit(self):
        ...
        filterReader = FilterReader(
            os.path.join(path_, "testcase01.conf"), "testcase01", {}
        )
        self.assertEqual(
...
class JailsReaderTestCache(LogCaptureTestCase):
    def _readWholeConf(self, basedir, force_enable=False, share_config=None):
        ...
        configurator = Configurator(
            force_enable=force_enable, share_config=share_config
        )
    ...
    def testTestJailConfCache(self, basedir):
        ...
        try:
            ...
            # 	print(self.getLog())
            self.assertTrue(
                cnt == 1, "Unexpected count by reading of jail files, cnt = %s" % cnt
            )
            ...
            self.assertTrue(
                cnt == 1,
                "Unexpected count by second reading of jail files, cnt = %s" % cnt,
            )

            # same with filter:
            cnt = self._getLoggedReadCount(r"filter\.d/common\.conf")
            self.assertTrue(
                cnt == 1, "Unexpected count by reading of filter files, cnt = %s" % cnt
            )
            # same with action:
            cnt = self._getLoggedReadCount(r"action\.d/iptables-common\.conf")
            self.assertTrue(
                cnt == 1, "Unexpected count by reading of action files, cnt = %s" % cnt
            )
        finally:
...
class JailsReaderTest(LogCaptureTestCase):
    ...
    def testReadTestJailConf(self):
        jails = JailsReader(
            basedir=IMPERFECT_CONFIG, share_config=IMPERFECT_CONFIG_SHARE_CFG
        )
        ...
        for actionConfig in glob.glob(os.path.join(CONFIG_DIR, "action.d", "*.conf")):
            ...
            if not actionName.endswith("-common"):
                ...
                self.assertEqual(
                    opts.get("name"),
                    "TEST",
                    msg="Action file %r does not contains jail-name 'f2b-TEST'"
                    % actionConfig,
                )
                ...
                if actionName in ("pf", "iptables-allports", "iptables-multiport"):
                    ...
                    self.assertIn(
                        ...
                        msg="Action file %r: interpolation of actionstart does not contains jail-name 'f2b-TEST'"
                        % actionConfig,
                    )
    ...
    def testReadStockJailConf(self):
        unittest.F2B.SkipIfCfgMissing(stock=True)
        jails = JailsReader(
            basedir=CONFIG_DIR, share_config=CONFIG_DIR_SHARE_CFG
        )  # we are running tests from root project dir atm
        ...
        for jail in jails.sections():
            ...
            self.assertTrue(
                filterReader.read(), "Failed to read filter:" + filterName
            )  # opens fine
    ...
    def testReadStockJailFilterComplete(self):
        unittest.F2B.SkipIfCfgMissing(stock=True)
        jails = JailsReader(
            basedir=CONFIG_DIR, force_enable=True, share_config=CONFIG_DIR_SHARE_CFG
        )
        self.assertTrue(jails.read())  # opens fine
        self.assertTrue(jails.getOptions())  # reads fine
    ...
    def testReadStockJailConfForceEnabled(self):
        ...
        for j in jails._JailsReader__jails:
            ...
            self.assertTrue(
                len(actions), msg="No actions found for jail %s" % jail_name
            )
            ...
            for action in actions:
                ...
                if "<blocktype>" in str(commands):
                    ...
                    for command in commands:
                        if (
                            len(command) > 4
                            and command[0] == "multi-set"
                            and command[1:4] == target_command
                        ):
                            blocktype_present = "blocktype" in [
                                cmd[0] for cmd in command[4]
                            ]
                        elif (
                            len(command) > 5
                            and command[0] == "set"
                            and command[1:4] == target_command
                            and command[4] == "blocktype"
        ...
        def find_set(option):
            for i, e in enumerate(commands):
                if e[0] == "set" and e[1] == option:
                    return i
            raise ValueError(
                "Did not find command 'set %s' among commands %s" % (option, commands)
            )
