...
from ..server.asyncserver import (
    asyncore,
    RequestHandler,
    loop,
    AsyncServer,
...
class Socket(LogCaptureTestCase):
    ...
    def _createServerThread(self, force=False):
        ...
        serverThread.start()
        self.assertTrue(
            Utils.wait_for(self.server.isActive, unittest.F2B.maxWaitTime(10))
        )
        return serverThread
    ...
    def _stopServerThread(self):
        ...
        Utils.wait_for(
            lambda: not serverThread.is_alive()
            or serverThread.join(Utils.DEFAULT_SLEEP_TIME),
            unittest.F2B.maxWaitTime(10),
        )
        self.serverThread = None
    ...
    def testSocket(self):
        ...
        self.assertLogged(
            "PROTO-error: load message failed:", "test unpickle error", all=True
        )
    ...
    def testStopByCommunicate(self):
        ...
        self.assertLogged(
            "Unexpected communication error", "test unpickle error", all=True
        )
    ...
    def testLoopErrors(self):
        ...
        try:
            ...
            self.assertTrue(
                Utils.wait_for(lambda: err["cntr"] > 50, unittest.F2B.maxWaitTime(10))
            )
        finally:
            # restore:
