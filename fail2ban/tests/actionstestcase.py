...
class ExecuteActions(LogCaptureTestCase):
    ...
    def testAddBannedIP(self):
        ...
        self.pruneLog()
        self.assertEqual(
            self.__actions.addBannedIP(["192.0.2.1", "192.0.2.2", "192.0.2.3"]), 2
        )
    ...
    def testAddActionPython(self):
        ...
        self.__actions.start()
        self.assertTrue(
            Utils.wait_for(lambda: self._is_logged("TestAction action start"), 3)
        )
        ...
        self.assertRaises(
            IOError, self.__actions.add, "Action3", "/does/not/exist.py", {}
        )
    ...
    def testAddPythonActionNOK(self):
        self.assertRaises(
            ...
            self.__actions.add,
            "Action",
            os.path.join(TEST_FILES_DIR, "action.d/action_noAction.py"),
            {},
        )
        self.assertRaises(
            RuntimeError,
            self.__actions.add,
            "Action",
            os.path.join(TEST_FILES_DIR, "action.d/action_nomethod.py"),
            {},
        )
        self.__actions.add(
            "Action", os.path.join(TEST_FILES_DIR, "action.d/action_errors.py"), {}
        )
        self.__actions.start()
    ...
    def testUnbanOnBusyBanBombing(self):
        ...
        MyTime.setTime(200)
        # unban time for 20 tickets reached
    ...
    def testActionsConsistencyCheckDiffFam(self):
        ...
        self.assertNotLogged(
            "stdout: %r" % "ip check inet4", "stdout: %r" % "ip start inet4", all=True
        )
    ...
    def testActionsRebanBrokenAfterRepair(self, tmp):
        ...
        act.actionreban = (
            'echo ip reban <ip> <family>; echo "<ip> <family> -- rebanned" >> "<FN>"'
        )
        ...
        self.assertEqual(
            self.__actions.addBannedIP(["192.0.2.1", "2001:db8::1", "2001:db8::2"]), 1
        )
        self.assertLogged(
            "192.0.2.1 already banned",
            "2001:db8::1 already banned",
            "Ban 2001:db8::2",
        ...
        self.assertLogged(
            ...
            "Reban 192.0.2.1, action 'ip'",
            "Reban 2001:db8::1, action 'ip'",  # both rebans also
            "stdout: %r" % "ip reban 192.0.2.1 inet4",
            "stdout: %r" % "ip reban 2001:db8::1 inet6",
            all=True,
        )
        ...
        self.dumpFile(tmp + "/inet6")
        self.assertLogged(
            "2001:db8::1 inet6 -- rebanned", "2001:db8::2 inet6 -- rebanned", all=True
        )
        ...
        self.assertLogged(
            "Failed to execute reban", "Error banning 192.0.2.1", all=True
        )
