...
RE_WRONG_GREED = re.compile(
    r"\.[+\*](?!\?)[^\$\^]*" + re.escape(RE_HOST) + r".*(?:\.[+\*].*|[^\$])$"
)
...
class FilterSamplesRegex(unittest.TestCase):
    ...
    def testFiltersPresent(self):
        ...
        self.assertTrue(
            len(
                [
                    test
                    for test in inspect.getmembers(self)
    ...
    def testReWrongGreedyCatchAll(self):
        ...
        self.assertTrue(
            RE_WRONG_GREED.search(
                "greedy .* test" + RE_HOST + " test not hard-anchored"
            )
        )
        self.assertTrue(
            RE_WRONG_GREED.search(
                "greedy .+ test" + RE_HOST + " test vary .* anchored$"
            )
        )
        self.assertFalse(
            RE_WRONG_GREED.search(
                "greedy .* test" + RE_HOST + " test no catch-all, hard-anchored$"
            )
        )
        self.assertFalse(
            RE_WRONG_GREED.search(
                "non-greedy .*? test" + RE_HOST + " test not hard-anchored"
            )
        )
        self.assertFalse(
            RE_WRONG_GREED.search(
                "non-greedy .+? test" + RE_HOST + " test vary catch-all .* anchored$"
            )
        )
    ...
    def _readFilter(self, fltName, name, basedir, opts=None):
        ...
        filterConf = FilterReader(
            name, "jail", opts, basedir=basedir, share_config=unittest.F2B.share_config
        )
        ...
        for fr in regexList:
            if RE_WRONG_GREED.search(fr):  # pragma: no cover
                raise AssertionError(
                    'Following regexp of "%s" contains greedy catch-all before <HOST>, '
    ...
    def testFilter(self):
        ...
        while i < len(filenames):
            ...
            logFile = fileinput.FileInput(
                os.path.join(TEST_FILES_DIR, "logs", filename), mode="rb"
            )

            ignoreBlock = False
            for line in logFile:
                line = FileContainer.decode_line(logFile.filename(), "UTF-8", line)
                jsonREMatch = re.match(
                    "^#+ ?(failJSON|(?:file|filter)Options|addFILE):(.+)$", line
                )
                if jsonREMatch:
                    try:
                        ...
                        if jsonREMatch.group(1) == "filterOptions":
                            ...
                            ignoreBlock = False
                            for faildata in (
                                faildata if isinstance(faildata, list) else [faildata]
                            ):
                                ...
                                if not ignoreBlock:
                                    ...
                                    # read it:
                                    flt = self._readFilter(
                                        fltName, name, basedir, opts=fltOpts
                                    )
                        ...
                        raise ValueError(
                            "%s: %s:%i" % (e, logFile.filename(), logFile.filelineno())
                        )
                    line = next(logFile)
                    line = FileContainer.decode_line(logFile.filename(), "UTF-8", line)
                ...
                for fltName, flt, opts in self._filterTests:
                    ...
                    try:
                        ...
                        if opts.get("logtype") != "journal":
                            ...
                            if opts.get(
                                "test.prefix-line"
                            ):  # journal backends creates common prefix-line:
                        ...
                        for ret in ret:
                            ...
                            if t is not None:
                                try:
                                    jsonTimeLocal = datetime.datetime.strptime(
                                        t, "%Y-%m-%dT%H:%M:%S"
                                    )
                                except ValueError:
                                    jsonTimeLocal = datetime.datetime.strptime(
                        ...
                        raise AssertionError(
                            ...
                            % (
                                ...
                                line,
                                failregex,
                                regexList[failregex] if failregex != -1 else None,
