...
__copyright__ = (
    "Copyright (c) 2014- Serg G. Brester (sebres), 2008- Fail2Ban Contributors"
)
__license__ = "GPL"
...
from ..client.fail2banclient import (
    exec_command_line as _exec_client,
    CSocket,
    VisualWait,
)
...
) = fail2banserver.output = protocol.output = _test_output
...
def _start_params(
    ...
    return (
        ...
        + (
            ...
            "auto",
            "--timeout",
            str(fail2bancmdline.MAX_WAITTIME),
        )
    )
        ...
        def wrapper(self, tmp, *args, **kwargs):
            ...
            try:
                ...
                def _stopAndWaitForServerEnd(code=(SUCCESS, FAILED)):
                    ...
                    if not phase.get("end", None) and not os.path.exists(
                        pjoin(tmp, "f2b.pid")
                    ):
                        Utils.wait_for(
                            lambda: phase.get("end", None) is not None, MID_WAITTIME
                        )
                    ...
                    if not phase.get("end", None):
                        ...
                        # wait for end sign:
                        Utils.wait_for(
                            lambda: phase.get("end", None) is not None, MAX_WAITTIME
                        )
                ...
                Utils.wait_for(
                    lambda: phase.get("start", None) is not None, MAX_WAITTIME
                )
    ...
    def _wait_for_srv(self, tmp, ready=True, startparams=None, phase=None):
        ...
        try:
            ...
            if ready:
                ...
                ret = Utils.wait_for(
                    lambda: "Server ready" in self.getLog(), MAX_WAITTIME
                )
...
class Fail2banClientTest(Fail2banClientServerBase):
    ...
    def testClientFailStart(self, tmp):
        ...
        self.assertLogged(
            "Base configuration directory " + pjoin(tmp, "miss") + " does not exist"
        )
        self.pruneLog()
...
class Fail2banServerTest(Fail2banClientServerBase):
    ...
    def testServerFailStart(self, tmp):
        ...
        open(pjoin(tmp, "f2b.sock"), "a").close()
        self.execCmd(
            FAILED, (), "-c", pjoin(tmp, "config"), "-s", pjoin(tmp, "f2b.sock")
        )
        self.assertLogged(
            "Fail2ban seems to be in unexpected state (not running but the socket exists)"
        )
    ...
    def testServerReloadTest(self, tmp, startparams):
        ...
        def _write_action_cfg(
            ...
            _write_file(
                ...
                "actionstart =  echo '[%(name)s] %(actname)s: ** start'",
                start,
                "actionreload = echo '[%(name)s] %(actname)s: .. reload'",
                reload,
                "actionban =    echo '[%(name)s] %(actname)s: ++ ban <ip> %(restore)s%(info)s'",
                ban,
                "actionunban =  echo '[%(name)s] %(actname)s: -- unban <ip>'",
                unban,
                "actionstop =   echo '[%(name)s] %(actname)s: __ stop'",
                stop,
            )
            if unittest.F2B.log_level <= logging.DEBUG:  # pragma: no cover
                _out_file(fn)
        ...
        self.assertLogged(
            ...
            "stdout: '[test-jail1] test-action2: ** start'",
            all=True,
        )
        ...
        self.assertLogged(
            ...
            "Jail 'broken-jail' skipped, because of wrong configuration",
            all=True,
        )
        ...
        self.assertNotLogged(
            "[test-jail1] Unban 192.0.2.1", "[test-jail1] Ban 192.0.2.1", all=True
        )
        # test 2 new log files:
        self.assertLogged(
            "Added logfile: %r" % test2log, "Added logfile: %r" % test3log, all=True
        )
        # test actions reloaded:
        self.assertLogged(
            "stdout: '[test-jail1] test-action1: .. reload'",
            "stdout: '[test-jail1] test-action2: .. reload'",
            all=True,
        )
        # test 1 new jail:
        self.assertLogged(
            "Creating new jail 'test-jail2'", "Jail 'test-jail2' started", all=True
        )
        # test action3 removed, test flushing successful (and no single unban occurred):
        self.assertLogged(
            "stdout: '[test-jail1] test-action3: -- flushing IPs'",
            "stdout: '[test-jail1] test-action3: __ stop'",
            all=True,
        )
        ...
        self.assertLogged(
            "stdout: '[test-jail1] test-action1: .. reload'",
            "stdout: '[test-jail1] test-action1: reloaded.'",
            all=True,
        )
        ...
        self.assertSortedEqual(
            self.execCmdDirect(startparams, "banned"),
            (
                0,
                [
                    {
        ...
        self.assertSortedEqual(
            ...
            (0, [["test-jail1"], ["test-jail1", "test-jail2"], []]),
        )
        self.assertSortedEqual(
        ...
        self.assertEqual(
            self.execCmdDirect(startparams, "get", "test-jail1", "banned", "192.0.2.3")[
                1
            ],
            1,
        )
        self.assertEqual(
            self.execCmdDirect(startparams, "get", "test-jail1", "banned", "192.0.2.9")[
                1
            ],
            0,
        )
        ...
        self.assertLogged(
            ...
            "[test-jail2] Restore Ban 192.0.2.9",
            all=True,
        )
        ...
        self.assertLogged(
            "Reload finished.", "Jail 'test-jail2' started", all=True, wait=MID_WAITTIME
        )
        self.assertLogged(
            "Jail 'test-jail2' stopped",
            "Jail 'test-jail2' started",
        ...
        self.assertLogged(
            ...
            "stdout: '[test-jail2] test-action2: -- unban 192.0.2.22'",
            all=True,
        )
        ...
        self.assertNotLogged(
            ...
            "stdout: '[test-jail2] test-action3: -- unban 192.0.2.22'",
            all=True,
        )
        # no more ban (unbanned all):
        self.assertNotLogged(
            "[test-jail2] Ban 192.0.2.4", "[test-jail2] Ban 192.0.2.8", all=True
        )
        ...
        self.assertLogged("Reload finished.", wait=MID_WAITTIME)
        self.assertLogged(
            "Reload jail 'test-jail1'", "Jail 'test-jail1' reloaded", all=True
        )
        self.assertNotLogged(
        ...
        self.assertLogged(
            "Stopping jail 'test-jail2'", "Jail 'test-jail2' stopped", all=True
        )
        # test 2 log files removed:
        self.assertLogged(
            "Removed logfile: %r" % test2log, "Removed logfile: %r" % test3log, all=True
        )
        ...
        self.execCmd(
            SUCCESS, startparams, "--async", "unban", "192.0.2.8/31", "192.0.2.100/31"
        )
        self.assertLogged(
            "[test-jail1] Unban 192.0.2.8",
            "192.0.2.100/31 is not banned",
            all=True,
            wait=MID_WAITTIME,
        )
        ...
        # contains both subnets .96/28 and .112/28
        self.assertLogged(
            "[test-jail1] Unban 192.0.2.96/28",
            "[test-jail1] Unban 192.0.2.112/28",
            all=True,
            wait=MID_WAITTIME,
        )
        ...
        self.assertLogged(
            "Flush ban list", "Unbanned 0, 0 ticket(s) in 'test-jail1'", all=True
        )
        ...
        self.execCmd(
            SUCCESS, startparams, "--async", "reload", "--if-exists", "test-jail2"
        )
        self.assertLogged("Reload finished.", wait=MID_WAITTIME)
        self.assertNotLogged(
            "Creating new jail 'test-jail2'", "Jail 'test-jail2' started", all=True
    ...
    def testServerActions_NginxBlockMap(self, tmp, startparams):
        ...
        _write_file(
            ...
            str(int(MyTime.time()))
            + ' failure "125-000-003" - 192.0.2.1 (\xf2\xf0\xe5\xf2\xe8\xe9)',
            str(int(MyTime.time()))
            + ' failure "125-000-004" - 192.0.2.1 (\xf2\xf0\xe5\xf2\xe8\xe9)',
        ...
        # unban 1, 2 and 5:
        self.execCmd(
            SUCCESS, startparams, "unban", "125-000-001", "125-000-002", "125-000-005"
        )
        _out_file(mpfn)
    ...
    @with_foreground_server_thread(
        startextra={
            ...
            "use_stock": True,
            # fail2ban.local:
            "f2b_local": ("[DEFAULT]", "dbmaxmatches = 1"),
            # jail.local config:
            "jails": (
                # default:
    ...
    def testServerJails_Sendmail(self, tmp, startparams):
        ...
        smaut_msg = (
            str(int(MyTime.time()))
            + " smtp1 sm-mta[5133]: s1000000000001: [192.0.2.1]: possible SMTP attack: command=AUTH, count=1",
            str(int(MyTime.time()))
            + " smtp1 sm-mta[5133]: s1000000000002: [192.0.2.1]: possible SMTP attack: command=AUTH, count=2",
            str(int(MyTime.time()))
            + " smtp1 sm-mta[5133]: s1000000000003: [192.0.2.1]: possible SMTP attack: command=AUTH, count=3",
        )
        smrej_msg = (
            str(int(MyTime.time()))
            + " smtp1 sm-mta[21134]: s2000000000001: ruleset=check_rcpt, arg1=<123@example.com>, relay=xxx.dynamic.example.com [192.0.2.2], reject=550 5.7.1 <123@example.com>... Relaying denied. Proper authentication required.",
            str(int(MyTime.time()))
            + " smtp1 sm-mta[21134]: s2000000000002: ruleset=check_rcpt, arg1=<345@example.com>, relay=xxx.dynamic.example.com [192.0.2.2], reject=550 5.7.1 <345@example.com>... Relaying denied. Proper authentication required.",
            str(int(MyTime.time()))
            + " smtp1 sm-mta[21134]: s3000000000003: ruleset=check_rcpt, arg1=<567@example.com>, relay=xxx.dynamic.example.com [192.0.2.2], reject=550 5.7.1 <567@example.com>... Relaying denied. Proper authentication required.",
        )
        ...
        self.assertLogged(
            "[sendmail-auth] Ban 192.0.2.1",
            "1 ticket(s) in 'sendmail-auth'",
            all=True,
            wait=MID_WAITTIME,
        )
        _out_file(tofn)
        ...
        self.assertLogged(
            "Reload finished.",
            "[sendmail-auth] Restore Ban 192.0.2.1",
            "1 ticket(s) in 'sendmail-auth'",
            all=True,
            wait=MID_WAITTIME,
        )
    ...
    def testServerObserver(self, tmp, startparams):
        ...
        _write_file(
            ...
            *(
                (
                    str(int(MyTime.time()))
                    + ' failure 401 from 192.0.2.11: I\'m bad "hacker" `` $(echo test)',
                )
                * 3
            )
        )
        ...
        _write_file(
            ...
            *(
                (
                    str(int(MyTime.time()))
                    + ' failure 401 from 192.0.2.11: I\'m very bad "hacker" `` $(echo test)',
                )
                * 2
            )
        )
        ...
        def _long_action():
            ...
            obsMain.db_purge()
            # does nothing (db is already None)
