...
class SetupTest(unittest.TestCase):
    def setUp(self):
        ...
        if not self.setup and sys.version_info >= (
            2,
            7,
        ):  # pragma: no cover - running not out of the source
        ...
        if sysVer != str(tuple(sys.version_info)):
            raise unittest.SkipTest(
                ...
                " -- install can be tested only with system distribution %s"
                % (str(tuple(sys.version_info)), sysVer)
            )
    ...
    def testSetupInstallDryRun(self):
        ...
        supdbgout = (
            " >/dev/null 2>&1" if unittest.F2B.log_level >= logging.DEBUG else ""
        )  # HEAVYDEBUG
        try:
            # try dry-run:
            os.system(
    ...
    def testSetupInstallRoot(self):
        ...
        # suppress stdout (and stderr) if not heavydebug
        supdbgout = (
            " >/dev/null" if unittest.F2B.log_level >= logging.DEBUG else ""
        )  # HEAVYDEBUG
        try:
            self.assertEqual(
                os.system(
            ...
            if set(need).difference(got):  # pragma: no cover
                ...
                for missing in set(got).difference(need):
                    ...
                    files[missing] = (
                        os.path.exists(missing_full)
                        and strippath(recursive_glob(missing_full, "*"))
                        or None
                    )
            ...
            for f in ("etc/fail2ban/fail2ban.conf", "etc/fail2ban/jail.conf"):
                self.assertTrue(
                    os.path.exists(os.path.join(tmp, f)), msg="Can't find %s" % f
                )
            ...
            os.system(
                ...
                % (
                    sys.executable,
                    self.setup,
                    (supdbgout + " 2>&1") if supdbgout else "",
                )
            )
...
class TestsUtilsTest(LogCaptureTestCase):
    ...
    def testSafeLogging(self):
        ...
        class Test:
            ...
            def __repr__(self):
                if self.err:
                    ...
                    return (
                        u"conv-error (\xf2\xf0\xe5\xf2\xe8\xe9), unterminated utf \xcf"
                    )
        ...
        self.pruneLog(
            "[phase 2] test error conversion by encoding %s" % sys.getdefaultencoding()
        )
        test = Test(0)
    ...
    def testExtendedAssertRaisesRE(self):
        ...
        self.assertRaises(
            KeyError, self._testAssertionErrorRE, r"^failed$", _key_err, "failed"
        )
        self.assertRaises(
            AssertionError,
        ...
        self._testAssertionErrorRE(
            ...
            lambda: self._testAssertionErrorRE(r"^failed$", self.fail, "__failed__"),
        )
        ## no exception in callable:
        self.assertRaises(AssertionError, self._testAssertionErrorRE, r"", int, 1)
        self._testAssertionErrorRE(
            r"0 AssertionError not raised X.* does not match .*AssertionError not raised",
            lambda: self._testAssertionErrorRE(
                r"^0 AssertionError not raised X$",
    ...
    def testExtendedAssertMethods(self):
        ...
        self._testAssertionErrorRE(
            r"'a' unexpectedly found in 'cba'", self.assertNotIn, "a", "cba"
        )
        self._testAssertionErrorRE(
            r"1 unexpectedly found in \[0, 1, 2\]", self.assertNotIn, 1, xrange(3)
        )
        self._testAssertionErrorRE(
            r"'A' unexpectedly found in \['C', 'A'\]",
            self.assertNotIn,
            "A",
            (c.upper() for c in "cba" if c != "b"),
        )
        self._testAssertionErrorRE(
            r"'a' was not found in 'xyz'", self.assertIn, "a", "xyz"
        )
        self._testAssertionErrorRE(
            r"5 was not found in \[0, 1, 2\]", self.assertIn, 5, xrange(3)
        )
        self._testAssertionErrorRE(
            r"'A' was not found in \['C', 'B'\]",
            self.assertIn,
            "A",
            (c.upper() for c in "cba" if c != "a"),
        )
        ...
        logSys.debug('test "xyz"')
        self._testAssertionErrorRE(
            r".* was found in the log", self.assertNotLogged, 'test "xyz"'
        )
        self._testAssertionErrorRE(
        ...
        self._testAssertionErrorRE(
            r"was found in the log", self.assertNotLogged, "test", "xyz", all=True
        )
        self._testAssertionErrorRE(
            r"was not found in the log", self.assertLogged, "test", "zyx", all=True
        )
        self._testAssertionErrorRE(
        ...
        self.assertRaises(
            AssertionError, self.assertDictEqual, {"A": [1, 2]}, {"A": [2, 1]}
        )
        ## assertSortedEqual:
        ...
        self.assertRaises(AssertionError, self.assertSortedEqual, [["ABC"]], [["CBA"]])
        self._testAssertionErrorRE(
            r"\['A'\] != \['C', 'B'\]", self.assertSortedEqual, ["A"], ["C", "B"]
        )
        self._testAssertionErrorRE(
            r"\['A', 'B'\] != \['B', 'C'\]",
            self.assertSortedEqual,
...
class MyTimeTest(unittest.TestCase):
    def testStr2Seconds(self):
        ...
        self.assertEqual(
            str2sec(
                "2 * 0.5 yea + 1*1 mon - 3*1/3 wee + 2/2 day - (2*12 hou 3*20 min 80 sec) "
            ),
            33578920.0,
        )
        self.assertEqual(
            str2sec("2*.5y+1*1mo-3*1/3w+2/2d-(2*12h3*20m80s) "), 33578920.0
        )
