...
class BanTimeIncr(LogCaptureTestCase):
    ...
    def testDefault(self, multipliers=None):
        ...
        self.assertEqual([a.calcBanTime(600, i) for i in xrange(1, 11)], arr)
        a.setBanTimeExtra("maxtime", "1d")
        # change factor :
        ...
        self.assertTrue(
            False
            in [1200 in [a.calcBanTime(600, 1) for i in xrange(10)] for c in xrange(10)]
        )
        a.setBanTimeExtra("rndtime", None)
        self.assertFalse(
            False
    ...
    def testFormula(self):
        ...
        self.assertTrue(
            False
            in [
                1200 in [int(a.calcBanTime(600, 1)) for i in xrange(10)]
                for c in xrange(10)
            ]
        )
        a.setBanTimeExtra("rndtime", None)
...
class BanTimeIncrDB(LogCaptureTestCase):
    ...
    def testBanTimeIncr(self):
        ...
        self.assertEqual(
            [self.incrBanTime(ticket, 10) for i in xrange(3)], [10, 10, 10]
        )
        # add a ticket banned
        ticket.incrBanCount()
        ...
        self.assertEqual(
            [
                ...
                for banCount, timeOfBan, lastBanTime in self.db.getBan(
                    ip, jail, None, False
                )
            ],
            [(1, stime, 10)],
        ...
        self.assertEqual(
            [
                (banCount, timeOfBan, lastBanTime)
                for banCount, timeOfBan, lastBanTime in self.db.getBan(
                    ip, forbantime=stime, fromtime=stime
                )
            ],
            [(2, stime + 15, 20)],
        )
        ...
        self.assertEqual(
            self.db.getCurrentBans(
                forbantime=-24 * 60 * 60, fromtime=stime, correctBanTime=False
            ),
            [],
        )
        ...
        self.assertEqual(
            ...
            (
                "FailTicket: ip=%s time=%s bantime=20 bancount=2 #attempts=0 matches=[]"
                % (ip, stime + 15)
            ),
        )
        ...
        restored_tickets = self.db.getCurrentBans(
            jail=jail, fromtime=stime, correctBanTime=False
        )
        self.assertEqual(
            str(restored_tickets),
            (
        ...
        self.assertEqual(
            ...
            "FailTicket: ip=%s time=%s bantime=%s bancount=13 #attempts=0 matches=[]"
            % (ip, stime + lastBanTime + 5, lastBanTime),
        )
        self.assertEqual(
            str(restored_tickets[1]),
            "FailTicket: ip=%s time=%s bantime=%s bancount=1 #attempts=0 matches=[]"
            % (ip + "1", stime - 24 * 60 * 60, 36 * 60 * 60),
        )
        # search out-dated (give another fromtime now is -18 hours):
        restored_tickets = self.db.getCurrentBans(
            fromtime=stime - 18 * 60 * 60, correctBanTime=False
        )
        ...
        restored_tickets = self.db.getCurrentBans(
            jail=jail1, fromtime=stime, correctBanTime=False
        )
        self.assertEqual(len(restored_tickets), 1)
        self.assertEqual(
            str(restored_tickets[0]),
        ...
        restored_tickets = self.db.getCurrentBans(
            jail=jail2, fromtime=stime, correctBanTime=False
        )
        self.assertEqual(len(restored_tickets), 1)
        self.assertEqual(
            str(restored_tickets[0]),
            "FailTicket: ip=%s time=%s bantime=%s bancount=2 #attempts=0 matches=[]"
            % (ip, stime - 6000, 12000),
        )
    ...
    def testObserver(self):
        ...
        restored_tickets = self.db.getCurrentBans(
            jail=jail, fromtime=stime - 120, correctBanTime=False
        )
        self.assertEqual(len(restored_tickets), 1)
