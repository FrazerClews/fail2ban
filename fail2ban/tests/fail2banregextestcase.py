...
__copyright__ = (
    "Copyright (c) 2015 Serg G. Brester (sebres), 2008- Fail2Ban Contributors"
)
__license__ = "GPL"
...
from ..client.fail2banregex import (
    Fail2banRegex,
    get_opt_parser,
    exec_command_line,
    output,
...
STR_ML_SSHD_FAIL = (
    "Nov 28 09:16:06 srv sshd[32307]: Connection closed by 192.0.2.1 [preauth]"
)
...
FILTER_ZZZ_SSHD = os.path.join(
    TEST_CONFIG_DIR, "filter.d", "zzz-sshd-obsolete-multiline.conf"
)
...
class Fail2banRegexTest(LogCaptureTestCase):
    ...
    def testWrongRE(self):
        self.assertFalse(_test_exec("test", r".** from <HOST>$"))
        self.assertLogged("Unable to compile regular expression")

    def testWrongIngnoreRE(self):
        self.assertFalse(
            _test_exec(
                "--datepattern", "{^LN-BEG}EPOCH", "test", r".*? from <HOST>$", r".**"
            )
        )
        self.assertLogged("Unable to compile regular expression")
    ...
    def testDirectNotFound(self):
        self.assertTrue(_test_exec("--print-all-missed", STR_00, r"XYZ from <HOST>$"))
    ...
    def testVerboseFullSshd(self):
        self.assertTrue(
            _test_exec(
                "-l",
                "notice",  # put down log-level, because of too many debug-messages
                "-v",
                "--verbose-date",
                "--print-all-matched",
                "--print-all-ignored",
        ...
        self.assertLogged(
            ...
            "[29116]: Received disconnect from 1.2.3.4",
            all=True,
        )
        self.pruneLog()
        # show real options:
    ...
    def testFastSshd(self):
        ...
        self.assertLogged(
            ...
            "[29116]: User root not allowed because account is locked",
            "[29116]: Received disconnect from 192.0.2.4",
            all=True,
        )
    ...
    def testMultilineSshd(self):
        ...
        self.assertTrue(
            _test_exec(
                ...
                "--print-all-missed",
                "-c",
                os.path.dirname(FILTER_ZZZ_SSHD),
                FILENAME_ZZZ_SSHD,
                os.path.basename(FILTER_ZZZ_SSHD),
            )
        )
    ...
    def testDirectMultilineBuf(self):
        ...
        for preLines in (0, 20):
            ...
            self.assertLogged(
                "Lines: %s lines, 0 ignored, 2 matched, %s missed"
                % (preLines + 4, preLines + 2)
            )
            # both matched lines were printed:
            self.assertLogged(
                "|  1490349000 FAIL", "|  1490349001 HOST 192.0.2.34", all=True
            )
    ...
    def testFrmtOutput(self):
        ...
        self.assertLogged(
            "['192.0.2.0'", "'ip4': '192.0.2.0'", "'user': 'kevin'", all=True
        )
        self.pruneLog()
        # log msg :
        ...
        self.assertTrue(
            _test_exec("-o", "<ip>, <F-USER>, <family>", STR_00, RE_00_USER)
        )
    ...
    def testNoDateTime(self):
        ...
        self.assertTrue(
            _test_exec("-d", "{NONE}", "-o", "Found-ID:<F-ID>", STR_00_NODT, RE_00_ID)
        )
    ...
    def testFrmtOutputWrapML(self):
        ...
        self.assertTrue(
            _test_exec(
                ...
                "no",
                STR_ML_SSHD + "\n" + STR_ML_SSHD_OK,
                "sshd[logtype=short, publickey=invalid]",
            )
        )
        ...
        self.assertLogged(
            "192.0.2.2, git, " + lines[-2], "192.0.2.2, git, " + lines[-1], all=True
        )
        ...
        self.pruneLog(
            "[test-phase 1] mode=aggressive & publickey=nofail + OK (accepted)"
        )
        self.assertTrue(
            _test_exec(
                "-o",
                ...
                "no",
                STR_ML_SSHD + "\n" + STR_ML_SSHD_OK,
                "sshd[logtype=short, mode=aggressive]",
            )
        )
        self.assertLogged(
        ...
        self.pruneLog(
            "[test-phase 2] mode=aggressive & publickey=nofail + FAIL (closed on preauth)"
        )
        self.assertTrue(
            _test_exec(
                "-o",
    ...
    def testWronChar(self):
        ...
        self.assertLogged(
            "Nov  8 00:16:12 main sshd[32548]: input_userauth_request: invalid user llinco"
        )
        self.assertLogged(
            "Nov  8 00:16:12 main sshd[32547]: pam_succeed_if(sshd:auth): error retrieving information about user llinco"
        )
    ...
    def testExecCmdLine_Direct(self):
        self.assertEqual(
            _test_exec_command_line(
                "-l", "info", STR_00, r"Authentication failure for .*? from <HOST>$"
            ),
            0,
        )
        self.assertLogged("Lines: 1 lines, 0 ignored, 1 matched, 0 missed")
    ...
    def testExecCmdLine_MissFailID(self):
        self.assertNotEqual(
            _test_exec_command_line("-l", "info", STR_00, r"Authentication failure"), 0
        )
    ...
    def testExecCmdLine_ErrorParam(self):
        # single line error:
        self.assertNotEqual(
            _test_exec_command_line("-l", "notice", "-d", "%:%.%-", "LOG", "RE"), 0
        )
        self.assertLogged("ERROR: Failed to set datepattern")
        # verbose (traceback/callstack):
        self.pruneLog()
        self.assertNotEqual(
            _test_exec_command_line("-v", "-d", "%:%.%-", "LOG", "RE"), 0
        )
