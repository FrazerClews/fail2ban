...
def skip_if_not_available(f):
    ...
    def wrapper(self, *args):
        try:
            ...
            URLError,
            timeout,
        ) as e:  # pragma: no cover - timeout/availability issues
...
if sys.version_info >= (2, 7):  # pragma: no cover - may be unavailable
    ...
    class BadIPsActionTest(LogCaptureTestCase):
        ...
        def setUp(self):
            ...
            if not BadIPsActionTest.available[0]:
                raise unittest.SkipTest(
                    "Skip test because service is not available: %s"
                    % BadIPsActionTest.available[1]
                )

            self.jail.actions.add(
                "badips",
                pythonModuleName,
                initOpts={
                    "category": "ssh",
        ...
        def testCategory(self):
            ...
            self.assertRaises(ValueError, setattr, self.action, "category", "mail")
        ...
        def testUpdateperiod(self):
            self.assertRaises(ValueError, setattr, self.action, "updateperiod", -50)
            self.assertRaises(ValueError, setattr, self.action, "updateperiod", 0)
        ...
        def testStartStop(self):
            self.action.start()
            self.assertTrue(
                len(self.action._bannedips) > 10,
                "%s is fewer as 10: %r"
                % (len(self.action._bannedips), self.action._bannedips),
            )
            self.action.stop()
        ...
        @skip_if_not_available
        def testBanIP(self):
            aInfo = CallingMap({"ip": IPAddr("192.0.2.1")})
            ...
            aInfo["ip"] = ""
            self.assertRaises(
                BadIPsActionTest.pythonModule.HTTPError, self.action.ban, aInfo
            )
