...
class SMTPActionTest(unittest.TestCase):
    ...
    def testStart(self):
        ...
        self.assertTrue(
            "Subject: [Fail2Ban] %s: started" % self.jail.name in self.smtpd.data
        )
    ...
    def testStop(self):
        ...
        self.assertEqual(self.smtpd.rcpttos, ["root"])
        self.assertTrue(
            "Subject: [Fail2Ban] %s: stopped" % self.jail.name in self.smtpd.data
        )
    ...
    def _testBan(self, restored=False):
        ...
        self.assertEqual(self.smtpd.mailfrom, "fail2ban")
        self.assertEqual(self.smtpd.rcpttos, ["root"])
        subject = "Subject: [Fail2Ban] %s: banned %s" % (self.jail.name, aInfo["ip"])
        self.assertIn(subject, self.smtpd.data)
        self.assertIn("%i attempts" % aInfo["failures"], self.smtpd.data)
    ...
    def testOptions(self):
        ...
        self.assertTrue(
            "From: %s <%s>" % (self.action.fromname, self.action.fromaddr)
            in self.smtpd.data
        )
        self.assertEqual(
            set(self.smtpd.rcpttos), set(["test@example.com", "test2@example.com"])
        )
