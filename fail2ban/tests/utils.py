...
def initProcess(opts):
    ...
    if verbosity is None:
        verbosity = (
            ...
            if llev <= logging.HEAVYDEBUG
            else 3
            if llev <= logging.DEBUG
            else 2
            if llev <= min(logging.INFO, logging.NOTICE)
            else 1
            if llev <= min(logging.WARNING, logging.ERROR)
            else 0  # if llev <= logging.CRITICAL
        )
    ...
    if opts.verbosity != 0:
        print(
            "Fail2ban %s test suite. Python %s. Please wait..."
        ...
        if not STOCK:  # pragma: no cover
            if kwargs.get("stock"):
                raise unittest.SkipTest(
                    "Skip test because of missing stock-config files"
                )
            for t in ("action", "filter"):
                ...
                if not os.path.exists(os.path.join(CONFIG_DIR, t + ".d", v)):
                    raise unittest.SkipTest(
                        "Skip test because of missing %s-config for %r" % (t, v)
                    )
...
def gatherTests(regexps=None, opts=None):
    ...
    for file_ in os.listdir(os.path.abspath(os.path.dirname(action_d.__file__))):
    ...
    try:
        ...
        if unittest.F2B.fast or unittest.F2B.no_gamin:  # pragma: no cover
            raise ImportError(
                "Skip, fast: %s, no_gamin: %s"
                % (unittest.F2B.fast, unittest.F2B.no_gamin)
            )
    ...
    for Filter_ in filters:
        tests.addTest(
            unittest.makeSuite(filtertestcase.get_monitor_failures_testcase(Filter_))
        )
    try:  # pragma: systemd no cover
        ...
        tests.addTest(
            unittest.makeSuite(
                filtertestcase.get_monitor_failures_journal_testcase(FilterSystemd)
            )
        )
    except ImportError as e:  # pragma: no cover
    ...
    def _assertSortedEqual(a, b, level, nestedOnly, key):
        ...
        if isinstance(a, dict) and isinstance(b, dict):  # compare dict's:
            for k, v1 in a.iteritems():
                v2 = b[k]
                if isinstance(v1, (dict, list, tuple)) and isinstance(
                    v2, (dict, list, tuple)
                ):
                    _assertSortedEqual(
                        v1, v2, level - 1 if level != 0 else 0, nestedOnly, key
                    )
                elif v1 != v2:
                    raise ValueError("%r != %r" % (a, b))
        ...
        def getvalue(self):
            ...
            if self._dirty & 2:
                ...
                try:
                    self._dirty &= (
                        ~3
                    )  # reset dirty records/buffer flag before cache value built
                    recs = self._recs
                    self._recs = list()
                finally:
                    self._lock.release()
        ...
        if not kwargs.get("all", False):
            ...
            if not res:
                ...
                self.fail(
                    ...
                    % (s, ((", waited %s" % wait) if wait else ""), logged)
                )
        else:
            # each entry should be found:
            if not res:
                ...
                for s_ in s:
                    if s_ not in logged:
                        self.fail(
                            ...
                            % (s_, ((", waited %s" % wait) if wait else ""), logged)
                        )
        ...
        if len(s) > 1 and not kwargs.get("all", False):
            for s_ in s:
                if s_ not in logged:
                    return
            self.fail(
                "All of the %r were found present in the log: ===\n%s===" % (s, logged)
            )
        else:
            for s_ in s:
                if s_ in logged:
