...
class TransmitterBase(LogCaptureTestCase):
    ...
    def setGetTest(
        self, cmd, inValue, outValue=(None,), outCode=0, jail=None, repr_=False
    ):
    ...
    def jailAddDelTest(self, cmd, values, jail):
        ...
        for n, value in enumerate(values):
            ...
            self.assertSortedEqual(
                (ret[0], map(str, ret[1])), (0, map(str, values[: n + 1])), level=2
            )
    ...
    def jailAddDelRegexTest(self, cmd, inValues, outValues, jail):
        ...
        for n, value in enumerate(inValues):
            ...
            self.assertEqual(
                self.transm.proceed(["get", jail, cmd]), (0, outValues[: n + 1])
            )
        for n, value in enumerate(inValues):
            self.assertEqual(
...
class Transmitter(TransmitterBase):
    ...
    def testStartStopJail(self):
        ...
        self.assertTrue(
            Utils.wait_for(
                lambda: self.server.isAlive(1)
                and not isinstance(
                    self.transm.proceed(["status", self.jailName]), RuntimeError
                ),
                3,
            )
        )
    ...
    def testJailIdle(self):
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "idle", "on"]), (0, True)
        )
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "idle", "off"]), (0, False)
        )
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "idle", "CAT"])[0], 1
        )
    ...
    def testJailUseDNS(self):
        ...
        value = "Fish"
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "usedns", value]), (0, "no")
        )
    ...
    def testJailBanIP(self):
        ...
        self.assertEqual(
            self.transm.proceed(
                ["set", self.jailName, "banip", "192.0.2.1", "192.0.2.1", "192.0.2.2"]
            ),
            (0, 2),
        )
        self.assertLogged(
            "Ban 192.0.2.1", "Ban 192.0.2.2", all=True, wait=True
        )  # Give chance to ban
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "banip", "Badger"]), (0, 1)
    ...
    def testJailAttemptIP(self):
        ...
        self.assertEqual(
            attempt(ip, ["test failure %d" % i for i in (3, 4, 5)]), (0, 1)
        )
    ...
    def testJailBanList(self):
        ...
        def _getBanListTest(jail, banip=None, unbanip=None, args=(), outList=[]):
            ...
            if banip is not None:
                self.assertEqual(
                    self.transm.proceed(["set", jail, "banip", banip]), (0, 1)
                )
                self.assertLogged("Ban %s" % banip, wait=True)  # Give chance to ban
            # Unban IP address
            if unbanip is not None:
                self.assertEqual(
                    self.transm.proceed(["set", jail, "unbanip", unbanip]), (0, 1)
                )
                self.assertLogged(
                    "Unban %s" % unbanip, wait=True
        ...
        _getBanListTest(
            jail, unbanip="127.0.0.1", outList=["192.168.0.1", "192.168.1.10"]
        )
    ...
    def testJailLogPath(self):
        ...
        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "logpath"]), (0, [value])
        )
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "dellogpath", value]), (0, [])
        )
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "addlogpath", value, "tail"]),
            (0, [value]),
        )
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "addlogpath", value, "head"]),
            (0, [value]),
        )
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "addlogpath", value, "badger"])[
                0
            ],
            1,
        )
        self.assertEqual(
            self.transm.proceed(
                ["set", self.jailName, "addlogpath", value, value, value]
            )[0],
            1,
        )
    ...
    def testJailIgnoreIP(self):
        ...
        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "ignoreip"]), (0, [value])
        )
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "delignoreip", value]), (0, [])
        )

        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "ignoreself"]), (0, True)
        )
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "ignoreself", False]), (0, False)
        )
        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "ignoreself"]), (0, False)
        )
    ...
    def testJailRegex(self):
        ...
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "addfailregex", 654])[0], 1
        )
    ...
    def testJailIgnoreRegex(self):
        ...
        self.assertEqual(
            self.transm.proceed(
                ["set", self.jailName, "addignoreregex", "Invalid [regex"]
            )[0],
            1,
        )
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "addignoreregex", 50])[0], 1
        )
    ...
    def testAction(self):
        ...
        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "actions"])[1][0], action
        )
        ...
        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "action", action, "KEY"]),
            (0, "VALUE"),
        )
        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "action", action, "InvalidKey"])[
                0
            ],
            1,
        )
        ...
        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "action", action, "timeout"]),
            (0, 10),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.transm.proceed(["set", self.jailName, "delaction", "Doesn't exist"])[
                0
            ],
            1,
        )
    ...
    def testPythonActionMethodsAndProperties(self):
        ...
        try:
            ...
            raise
        self.assertSortedEqual(
            self.transm.proceed(["get", self.jailName, "actionproperties", action])[1],
            ["opt1", "opt2"],
        )
        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "action", action, "opt1"]),
            (0, "value"),
        )
        self.assertEqual(
            self.transm.proceed(["get", self.jailName, "action", action, "opt2"]),
            (0, None),
        )
        self.assertSortedEqual(
            self.transm.proceed(["get", self.jailName, "actionmethods", action])[1],
    ...
    def testJournalMatch(self):  # pragma: systemd no cover
        ...
        for n, value in enumerate(values):
            self.assertEqual(
                self.transm.proceed(["set", jailName, "addjournalmatch", value]),
                (0, [[val] for val in values[: n + 1]]),
            )
        for n, value in enumerate(values):
            self.assertEqual(
                self.transm.proceed(["set", jailName, "deljournalmatch", value]),
                (0, [[val] for val in values[n + 1 :]]),
            )
        ...
        self.assertEqual(
            self.transm.proceed(["set", jailName, "deljournalmatch", value]), (0, [])
        )
        ...
        self.assertEqual(
            self.transm.proceed(["set", jailName, "deljournalmatch"] + value[:1]),
            (0, [["_SYSTEMD_UNIT=sshd.service", "_UID=0"]]),
        )
        self.assertEqual(
            self.transm.proceed(["set", jailName, "deljournalmatch"] + value[2:]),
            (0, []),
        )
...
class TransmitterLogging(TransmitterBase):
    ...
    def testFlushLogs(self):
        ...
        try:
                ...
                with open(fn2, "r") as f:
                    ...
                    try:
                        ...
                        if n.find("Command: ['flushlogs']") >= 0:
                            ...
                            self.fail(
                                "Exception StopIteration or Command: ['flushlogs'] expected. Got: %s"
                                % n
                            )
            ...
            try:
                ...
                pass
        self.assertEqual(
            self.transm.proceed(["set", "logtarget", "STDERR"]), (0, "STDERR")
        )
    ...
    def testBanTimeIncr(self):
        ...
        self.setGetTest(
            "bantime.maxtime", "1000 days", 1000 * 24 * 60 * 60, jail=self.jailName
        )
...
class RegexTests(unittest.TestCase):
    ...
    def testHost(self):
        ...
        self.assertTrue(
            FailRegex(r"^test id group: ip:port = <F-ID><IP4>(?::<F-PORT/>)?</F-ID>$")
        )
...
class LoggingTests(LogCaptureTestCase):
    ...
    def testFail2BanExceptHook(self):
        ...
        try:
            ...
            self.assertTrue(
                Utils.wait_for(
                    lambda: len(x) and self._is_logged("Unhandled exception"), 3
                )
            )
        finally:
...
class ServerConfigReaderTests(LogCaptureTestCase):
    ...
    def testCheckStockJailActions(self):
        ...
        jails = JailsReader(
            basedir=CONFIG_DIR, force_enable=True, share_config=self.__share_cfg
        )
        ...
        for cmd in stream:
            if cmd[0] != "start":
                ...
                # (we don't use it in this test at all):
                    ...
                    len(cmd) > 3
                    and cmd[0] in ("set", "multi-set")
                    and cmd[2] == "addfailregex"
                ):  # pragma: no cover
                    cmd[0] = "set"
    ...
    def getDefaultJailStream(self, jail, act):
        ...
        action = ActionReader(
            actName, jail, actOpt, share_config=self.__share_cfg, basedir=CONFIG_DIR
        )
    ...
    def testCheckStockCommandActions(self):
        ...
        testJailsActions = (
            ...
            (
                "j-w-nft-mp",
                'nftables-multiport[name=%(__name__)s, port="http,https", protocol="tcp,udp,sctp"]',
                {
                    "ip4": ("ip ", "ipv4_addr", "addr-"),
                    "ip6": ("ip6 ", "ipv6_addr", "addr6-"),
                    "*-start": (
            ...
            (
                "j-w-nft-ap",
                'nftables-allports[name=%(__name__)s, protocol="tcp,udp"]',
                {
                    "ip4": ("ip ", "ipv4_addr", "addr-"),
                    "ip6": ("ip6 ", "ipv6_addr", "addr6-"),
                    "*-start": (
            ...
            (
                "j-dummy",
                'dummy[name=%(__name__)s, init="==", target="/tmp/fail2ban.dummy"]',
                {
                    "ip4": ("family: inet4",),
                    "ip6": ("family: inet6",),
                    "start": (
            ...
            (
                "j-hostsdeny",
                'hostsdeny[name=%(__name__)s, actionstop="rm <file>", file="/tmp/fail2ban.dummy"]',
                {
                    "ip4": ("family: inet4",),
                    "ip6": ("family: inet6",),
                    "ip4-ban": (
            ...
            (
                "j-w-iptables-mp",
                'iptables-multiport[name=%(__name__)s, bantime="10m", port="http,https", protocol="tcp", chain="<known/chain>"]',
                {
                    "ip4": ("`iptables ", "icmp-port-unreachable"),
                    "ip6": ("`ip6tables ", "icmp6-port-unreachable"),
                    "ip4-start": (
            ...
            (
                "j-w-iptables-ap",
                'iptables-allports[name=%(__name__)s, bantime="10m", protocol="tcp", chain="<known/chain>"]',
                {
                    "ip4": ("`iptables ", "icmp-port-unreachable"),
                    "ip6": ("`ip6tables ", "icmp6-port-unreachable"),
                    "ip4-start": (
            ...
            (
                "j-w-iptables-ipset",
                'iptables-ipset-proto6[name=%(__name__)s, port="http", protocol="tcp", chain="<known/chain>"]',
                {
                    "ip4": (" f2b-j-w-iptables-ipset ",),
                    "ip6": (" f2b-j-w-iptables-ipset6 ",),
                    "ip4-start": (
            ...
            (
                "j-w-iptables-ipset-ap",
                'iptables-ipset-proto6-allports[name=%(__name__)s, chain="<known/chain>"]',
                {
                    "ip4": (" f2b-j-w-iptables-ipset-ap ",),
                    "ip6": (" f2b-j-w-iptables-ipset-ap6 ",),
                    "ip4-start": (
            ...
            (
                "j-w-iptables",
                'iptables[name=%(__name__)s, bantime="10m", port="http", protocol="tcp", chain="<known/chain>"]',
                {
                    "ip4": ("`iptables ", "icmp-port-unreachable"),
                    "ip6": ("`ip6tables ", "icmp6-port-unreachable"),
                    "ip4-start": (
            ...
            (
                "j-w-iptables-new",
                'iptables-new[name=%(__name__)s, bantime="10m", port="http", protocol="tcp", chain="<known/chain>"]',
                {
                    "ip4": ("`iptables ", "icmp-port-unreachable"),
                    "ip6": ("`ip6tables ", "icmp6-port-unreachable"),
                    "ip4-start": (
            ...
            (
                "j-w-iptables-xtre",
                'iptables-xt_recent-echo[name=%(__name__)s, bantime="10m", chain="<known/chain>"]',
                {
                    "ip4": ("`iptables ", "/f2b-j-w-iptables-xtre`"),
                    "ip6": ("`ip6tables ", "/f2b-j-w-iptables-xtre6`"),
                    "ip4-start": (
            ...
            (
                "j-w-pf",
                "pf[name=%(__name__)s, actionstart_on_demand=false]",
                {
                    "ip4": (),
                    "ip6": (),
                    "start": (
                    ...
                    "flush": ("`pfctl -a f2b/j-w-pf -t f2b-j-w-pf -T flush`",),
                    "stop": (
                    ...
                    "ip4-ban": (
                        "`pfctl -a f2b/j-w-pf -t f2b-j-w-pf -T add 192.0.2.1`",
                    ),
                    "ip4-unban": (
                        "`pfctl -a f2b/j-w-pf -t f2b-j-w-pf -T delete 192.0.2.1`",
                    ),
                    "ip6-ban": (
                        "`pfctl -a f2b/j-w-pf -t f2b-j-w-pf -T add 2001:db8::`",
                    ),
                    "ip6-unban": (
                        "`pfctl -a f2b/j-w-pf -t f2b-j-w-pf -T delete 2001:db8::`",
                    ),
                },
            ),
            ...
            (
                "j-w-pf-mp",
                'pf[actiontype=<multiport>][name=%(__name__)s, port="http,https"]',
                {
                    "ip4": (),
                    "ip6": (),
                    "start": (
                    ...
                    "flush": ("`pfctl -a f2b/j-w-pf-mp -t f2b-j-w-pf-mp -T flush`",),
                    "stop": (
                    ...
                    "ip4-check": (
                        "`pfctl -a f2b/j-w-pf-mp -sr | grep -q f2b-j-w-pf-mp`",
                    ),
                    "ip6-check": (
                        "`pfctl -a f2b/j-w-pf-mp -sr | grep -q f2b-j-w-pf-mp`",
                    ),
                    "ip4-ban": (
                        "`pfctl -a f2b/j-w-pf-mp -t f2b-j-w-pf-mp -T add 192.0.2.1`",
                    ),
                    "ip4-unban": (
                        "`pfctl -a f2b/j-w-pf-mp -t f2b-j-w-pf-mp -T delete 192.0.2.1`",
                    ),
                    "ip6-ban": (
                        "`pfctl -a f2b/j-w-pf-mp -t f2b-j-w-pf-mp -T add 2001:db8::`",
                    ),
                    "ip6-unban": (
                        "`pfctl -a f2b/j-w-pf-mp -t f2b-j-w-pf-mp -T delete 2001:db8::`",
                    ),
                },
            ),
            ...
            (
                "j-w-pf-ap",
                "pf[actiontype=<allports>, actionstart_on_demand=true][name=%(__name__)s]",
                {
                    "ip4": (),
                    "ip6": (),
                    "ip4-start": (
                    ...
                    "flush": ("`pfctl -a f2b/j-w-pf-ap -t f2b-j-w-pf-ap -T flush`",),
                    "stop": (
                    ...
                    "ip4-check": (
                        "`pfctl -a f2b/j-w-pf-ap -sr | grep -q f2b-j-w-pf-ap`",
                    ),
                    "ip6-check": (
                        "`pfctl -a f2b/j-w-pf-ap -sr | grep -q f2b-j-w-pf-ap`",
                    ),
                    "ip4-ban": (
                        "`pfctl -a f2b/j-w-pf-ap -t f2b-j-w-pf-ap -T add 192.0.2.1`",
                    ),
                    "ip4-unban": (
                        "`pfctl -a f2b/j-w-pf-ap -t f2b-j-w-pf-ap -T delete 192.0.2.1`",
                    ),
                    "ip6-ban": (
                        "`pfctl -a f2b/j-w-pf-ap -t f2b-j-w-pf-ap -T add 2001:db8::`",
                    ),
                    "ip6-unban": (
                        "`pfctl -a f2b/j-w-pf-ap -t f2b-j-w-pf-ap -T delete 2001:db8::`",
                    ),
                },
            ),
            ...
            (
                "j-w-fwcmd-mp",
                'firewallcmd-multiport[name=%(__name__)s, bantime="10m", port="http,https", protocol="tcp", chain="<known/chain>"]',
                {
                    "ip4": (" ipv4 ", "icmp-port-unreachable"),
                    "ip6": (" ipv6 ", "icmp6-port-unreachable"),
                    "ip4-start": (
            ...
            (
                "j-w-fwcmd-ap",
                'firewallcmd-allports[name=%(__name__)s, bantime="10m", protocol="tcp", chain="<known/chain>"]',
                {
                    "ip4": (" ipv4 ", "icmp-port-unreachable"),
                    "ip6": (" ipv6 ", "icmp6-port-unreachable"),
                    "ip4-start": (
            ...
            (
                "j-w-fwcmd-ipset",
                'firewallcmd-ipset[name=%(__name__)s, port="http", protocol="tcp", chain="<known/chain>"]',
                {
                    "ip4": (" f2b-j-w-fwcmd-ipset ",),
                    "ip6": (" f2b-j-w-fwcmd-ipset6 ",),
                    "ip4-start": (
                    ...
                    "ip4-unban": (r"`ipset del f2b-j-w-fwcmd-ipset 192.0.2.1 -exist`",),
                    "ip6-ban": (
            ...
            (
                "j-w-fwcmd-ipset-ap",
                'firewallcmd-ipset[name=%(__name__)s, actiontype=<allports>, protocol="tcp", chain="<known/chain>"]',
                {
                    "ip4": (" f2b-j-w-fwcmd-ipset-ap ",),
                    "ip6": (" f2b-j-w-fwcmd-ipset-ap6 ",),
                    "ip4-start": (
            ...
            for a in jails[jail].actions:
                ...
                if tests.get("start"):
                    ...
                    self.assertNotLogged(
                        *tests["ip4-start"] + tests["ip6-start"], all=True
                    )
                # test ban ip4 :
                ...
                if tests.get("ip4-start"):
                    self.assertLogged(
                        *tests.get("*-start", ()) + tests["ip4-start"], all=True
                    )
                ...
                if tests.get("ip6-start"):
                    self.assertLogged(
                        *tests.get("*-start", ()) + tests["ip6-start"], all=True
                    )
                if tests.get("ip4-start"):
                ...
                if tests.get("ip4-check"):
                    ...
                    if (
                        tests.get("ip6-check")
                        and tests["ip6-check"] != tests["ip4-check"]
                    ):
                ...
                if tests.get("ip6-check"):
                    ...
                    if (
                        tests.get("ip4-check")
                        and tests["ip4-check"] != tests["ip6-check"]
                    ):
    ...
    def testComplexMailActionMultiLog(self):
        ...
        testJailsActions = (
            ...
            (
                "j-xarf-abuse",
                "xarf-login-attack[" 'name=%(__name__)s, mailcmd="mail", mailargs="",' +
                # test reverse ip:
                "debug=1" + "]",
                {
                    "ip4-ban": (
