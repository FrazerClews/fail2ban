...
class DatabaseTest(LogCaptureTestCase):
    ...
    def testCreateInvalidPath(self):
        self.assertRaises(
            sqlite3.OperationalError, Fail2BanDb, "/this/path/should/not/exist"
        )
    ...
    def testUpdateDb(self):
        ...
        try:
            ...
            self.db = Fail2BanDb(self.dbFilename)
            self.assertEqual(
                self.db.getJailNames(), set(["DummyJail #29162448 with 0 tickets"])
            )
            ...
            self.assertEqual(
                self.db.updateDb(Fail2BanDb.__version__), Fail2BanDb.__version__
            )
            self.assertRaises(
                NotImplementedError, self.db.updateDb, Fail2BanDb.__version__ + 1
            )
            ...
            # ban-time was unknown (normally updated from jail)
        finally:
            if self.db and self.db._dbFilename != ":memory:":
                os.remove(self.db._dbBackupFilename)
    ...
    def testAddJail(self):
        ...
        self.assertTrue(
            self.jail.name in self.db.getJailNames(True), "Jail not added to database"
        )
    ...
    def testAddBanInvalidEncoded(self):
        ...
        tickets = [
            FailTicket(
                ...
                [
                    'user "test"',
                    'user "\xd1\xe2\xe5\xf2\xe0"',
                    'user "\xc3\xa4\xc3\xb6\xc3\xbc\xc3\x9f"',
                ],
            ),
            FailTicket(
                "127.0.0.2",
                ...
                [
                    'user "test"',
                    u'user "\xd1\xe2\xe5\xf2\xe0"',
                    u'user "\xc3\xa4\xc3\xb6\xc3\xbc\xc3\x9f"',
                ],
            ),
            FailTicket(
                "127.0.0.3",
                ...
                [
                    'user "test"',
                    b'user "\xd1\xe2\xe5\xf2\xe0"',
                    b'user "\xc3\xa4\xc3\xb6\xc3\xbc\xc3\x9f"',
                ],
            ),
            FailTicket(
                "127.0.0.4",
    ...
    def testGetBansMerged_MaxMatches(self):
        ...
        ticket = self.db.getCurrentBans(
            self.jail, "127.0.0.1", fromtime=MyTime.time() - 100, maxmatches=1
        )
        self.assertEqual(len(ticket.getMatches()), 1)
        self.assertEqual(ticket.getMatches(), failures[3]["matches"])
        ticket = self.db.getCurrentBans(
            self.jail, "127.0.0.1", fromtime=MyTime.time() - 100, maxmatches=0
        )
    ...
    def testGetBansMerged(self):
        ...
        self.assertEqual(
            id(ticket), id(self.db.getBansMerged("127.0.0.1", jail=self.jail))
        )
        ...
        tickets = self.db.getCurrentBans(
            jail=self.jail, forbantime=15, fromtime=MyTime.time()
        )
        self.assertEqual(len(tickets), 1)
        ...
        self.assertEqual(len(tickets), 0)
        self.assertLogged(
            "ignore ticket (with new max ban-time %r)" % self.jail.getMaxBanTime()
        )
