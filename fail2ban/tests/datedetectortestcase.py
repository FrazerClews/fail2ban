...
class DateDetectorTest(LogCaptureTestCase):
    ...
    def testGetEpochTime(self):
        ...
        for dateUnix in (1138049999, 32535244799):
            for date in ("%s", "[%s]", "[%s.555]", "audit(%s.555:101)"):
                ...
                self.assertIn(
                    matchlog.group(1), (str(dateUnix), str(dateUnix) + ".555")
                )
    ...
    def testVariousTimes(self):
        ...
        for anchored, bound, sdate, rdate in (
            ...
            (
                False,
                True,
                "01-23-2005 21:59:59.252",
                None,
            ),  # reported on f2b, causes Feb29 fix to break
            ...
            (
                ...
                "[1106513999.000]",
                "1106513999.000",
            ),  # epoch squared (brackets are not in match)
    ...
    def testAllUniqueTemplateNames(self):
        self.assertRaises(
            ValueError, self.datedetector.appendTemplate, self.datedetector.templates[0]
        )
    ...
    def testFullYearMatch_gh130(self):
        ...
        for i in xrange(10):
            (logTime, logMatch) = self.datedetector.getTime(
                "11/10/2012 02:37:17 [error] 18434#0"
            )
            self.assertEqual(logTime, mu)
            self.assertEqual(logMatch.group(1), "11/10/2012 02:37:17")
        # and now back to the original
        (logTime, logMatch) = self.datedetector.getTime(
            "2012/10/11 02:37:17 [error] 18434#0"
        )
        self.assertEqual(logTime, mu)
        self.assertEqual(logMatch.group(1), "2012/10/11 02:37:17")
    ...
    def testAmbiguousInOrderedTemplates(self):
        ...
        for (debit, line, cnt) in (
            ...
            (
                "030324  0:03:59",
                "some free text 030324  0:03:59 -- 2003-03-07 17:05:01 ...",
                1,
            ),
            ...
            (
                "030324  0:04:00",
                "server mysqld[1000]: 030324  0:04:00 [Warning] Access denied ..."
                " foreign-input just some free text 2003-03-07 17:05:01 test",
                10,
            ),
            # distance collision detection (first date should be found):
            (
                "Sep 16 21:30:26",
                "server mysqld[1020]: Sep 16 21:30:26 server mysqld: 030916 21:30:26 [Warning] Access denied",
                15,
            ),
            # just to test sorting:
            (
                "2005-10-07 06:09:42",
                "server mysqld[5906]: 2005-10-07 06:09:42 5907 [Warning] Access denied",
                20,
            ),
            (
                "2005-10-08T15:26:18.237955",
                "server mysqld[5906]: 2005-10-08T15:26:18.237955 6 [Note] Access denied",
                20,
            ),
            # date format changed again:
            (
                "051009 10:05:30",
                "server mysqld[1000]: 051009 10:05:30 [Warning] Access denied ...",
                50,
            ),
        ):
    ...
    def testLowLevelLogging(self):
        ...
        try:
            ...
            for (line, cnt) in (
                (
                    "server mysqld[5906]: 2005-10-07 06:09:%02i 5907 [Warning] Access denied",
                    2,
                ),
                (
                    "server mysqld[5906]: 051007 06:10:%02i 5907 [Warning] Access denied",
                    5,
                ),
                (
                    "server mysqld[5906]: 2005-10-07 06:09:%02i 5907 [Warning] Access denied",
    ...
    def testAmbiguousDatePattern(self):
        ...
        for (matched, dp, line) in (
            ...
            (
                "Aug 8 11:25:50",
                None,
                "Aug 8 11:25:50 20030f2329b8 Authentication failed from 192.0.2.1",
            ),
            (
                "Aug 8 11:25:50",
                None,
            ...
            (
                "Aug 8 11:25:50 2014",
                None,
                "Aug 8 11:25:50 2014 20030f2329b8 Authentication failed from 192.0.2.1",
            ),
            # direct specified patterns:
            (
                "20:00:00 01.02.2003",
                r"%H:%M:%S %d.%m.%Y$",
                "192.0.2.1 at 20:00:00 01.02.2003",
            ),
            (
                "[20:00:00 01.02.2003]",
                r"\[%H:%M:%S %d.%m.%Y\]",
                "192.0.2.1[20:00:00 01.02.2003]",
            ),
            (
                "[20:00:00 01.02.2003]",
                r"\[%H:%M:%S %d.%m.%Y\]",
                "[20:00:00 01.02.2003]192.0.2.1",
            ),
            (
                "[20:00:00 01.02.2003]",
                r"\[%H:%M:%S %d.%m.%Y\]$",
            ...
            (
                "20:00:00 01.02.2003",
                r"**%H:%M:%S %d.%m.%Y**",
                "192.0.2.1x20:00:00 01.02.2003",
            ),
            (
                "20:00:00 01.02.2003",
                r"**%H:%M:%S %d.%m.%Y**",
                "20:00:00 01.02.2003x192.0.2.1",
            ),
            # pattern enclosed with stars (in comparison to example above):
            (
                "*20:00:00 01.02.2003*",
                r"\**%H:%M:%S %d.%m.%Y\**",
                "test*20:00:00 01.02.2003*test",
            ),
            ...
            (
                None,
                r"%Y-%Exm-%Exd %ExH:%ExM:%ExS",
                "0000-12-30 00:00:00 - 2003-12-30 00:00:00",
            ),
            (
                "2003-12-30 00:00:00",
                r"%ExY-%Exm-%Exd %ExH:%ExM:%ExS",
                "0000-12-30 00:00:00 - 2003-12-30 00:00:00",
            ),
            ...
            (
                "200333 010203",
                r"%Y%m%d %H%M%S",
                "text:200333 010203 | date:20031230 010203",
            ),
            (
                "20031230 010203",
                r"%ExY%Exm%Exd %ExH%ExM%ExS",
            ...
            (
                "20030101 000000",
                "%ExY%Exm%Exd %ExH%ExM%ExS",
                "00001230 010203 - 20030101 000000",
            ),
            (
                None,
                "{^LN-BEG}%ExY%Exm%Exd %ExH%ExM%ExS",
                "00001230 010203 - 20030101 000000",
            ),
            (
                "20031230 010203",
                "{^LN-BEG}%ExY%Exm%Exd %ExH%ExM%ExS",
                "20031230 010203 - 20030101 000000",
            ),
            # Explicit bound in start of the line using {^LN-BEG} key,
            ...
            (
                "20031230010203",
                "{^LN-BEG}%ExY%Exm%Exd%ExH%ExM%ExS",
                "[20031230010203]20030101000000",
            ),
            # UTC/GMT time zone offset (with %z and %Z):
            (
                1072746123.0 - 3600,
                "{^LN-BEG}%ExY-%Exm-%Exd %ExH:%ExM:%ExS(?: %z)?",
                "[2003-12-30 01:02:03] server ...",
            ),
            (
                1072746123.0 - 3600,
                "{^LN-BEG}%ExY-%Exm-%Exd %ExH:%ExM:%ExS(?: %Z)?",
