...
class AddFailure(unittest.TestCase):
    ...
    def testAddDuplicateWithTime(self):
        ...
        for tnew, btnew in (
            ...
            self.assertEqual(
                banticket.getEndOfBanTime(defBanTime),
                ticket2.getEndOfBanTime(defBanTime),
            )
    ...
    def testUnban(self):
        ...
        self.assertEqual(
            self.__banManager.unBanList(stime + btime + 1), [self.__ticket]
        )
        ...
        self.assertEqual(
            len(self.__banManager.unBanList(stime + btime + 1 * 10 + 1)), 2
        )
        self.assertEqual(
            len(self.__banManager.unBanList(stime + btime + 5 * 10 + 1)), 3
        )
        self.assertEqual(self.__banManager.size(), 0)
    ...
    def testUnbanPermanent(self):
        ...
        try:
            ...
            self.assertTrue(self.__banManager._inBanList(self.__ticket))
            self.assertEqual(
                self.__banManager.unBanList(self.__ticket.getTime() + btime + 1), []
            )
            self.assertEqual(self.__banManager.size(), 1)
        finally:
...
class StatusExtendedCymruInfo(unittest.TestCase):
    ...
    def _getBanListExtendedCymruInfo(self):
        ...
        if cymru_info.get("error"):  # pragma: no cover - availability
            ...
            raise unittest.SkipTest(
                "Skip test because service is not available: %s" % cymru_info["error"]
            )
        return cymru_info
    ...
    def testCymruInfo(self):
        ...
        self.assertDictEqual(
            cymru_info,
            {"asn": [self.__asn], "country": [self.__country], "rir": [self.__rir]},
        )
    ...
    def testCymruInfoCountry(self):
        self.assertEqual(
            self.__banManager.geBanListExtendedCountry(
                self._getBanListExtendedCymruInfo()
            ),
            [self.__country],
        )
    ...
    def testCymruInfoNxdomain(self):
        ...
        self.assertDictEqual(
            cymru_info,
            {"asn": ["nxdomain"], "country": ["nxdomain"], "rir": ["nxdomain"]},
        )
