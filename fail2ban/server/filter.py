...
class Filter(JailThread):
    ...
    def delFailRegex(self, index=None):
        try:
            ...
            logSys.error(
                "Cannot remove regular expression. Index %d is not " "valid", index
            )
    ...
    def setUseDns(self, value):
        ...
        if not (value in ("yes", "warn", "no", "raw")):
            logSys.error(
                "Incorrect value %r specified for usedns. " "Using safe 'no'", value
            )
            value = "no"
    ...
    def setLogTimeZone(self, tz):
        validateTimeZone(tz)
        # avoid setting of wrong value, but hold original
    ...
    def ignoreCache(self):
        return (
            [
                self.__ignoreCache[0],
                self.__ignoreCache[1].maxCount,
                self.__ignoreCache[1].maxTime,
            ]
    ...
    def ignoreCache(self, command):
        if command:
            self.__ignoreCache = command["key"], Utils.Cache(
                maxCount=int(command.get("max-count", 100)),
                maxTime=MyTime.str2seconds(command.get("max-time", 5 * 60)),
            )
        else:
            self.__ignoreCache = None
    ...
    def addIgnoreIP(self, ipstr):
        ...
        if ip in self.__ignoreIpSet or ip in self.__ignoreIpList:
            logSys.log(
                logging.MSG,
                "  Ignore duplicate %r (%r), already in ignore list",
                ip,
                ipstr,
            )
            return
    ...
    def _inIgnoreIPList(self, ip, ticket, log_ignore=True):
        ...
        for net in self.__ignoreIpList:
            if ip.isInNet(net):
                self.logIgnoreIp(
                    ip, log_ignore, ignore_source=("ip" if net.isValid else "dns")
                )
                if self.__ignoreCache:
                    c.set(key, True)
                return True
        ...
        try:
            for element in self.processLine(line, date):
                ...
                if Observers.Main is not None:
                    Observers.Main.add(
                        "failureFound", self.failManager, self.jail, tick
                    )
    ...
    def _mergeFailure(self, mlfid, fail, failRegex):
        ...
        if (nfflgs & 1) == 0:  # current nofail state (corresponding users)
            ...
            fail["nofail-matches"] = (
                fail.get("nofail-matches", []) + failRegex.getMatchedTupleLines()
            )
        # return merged:
        return fail
    ...
    def findFailure(self, tupleLine, date, noDate=False):
        ...
        for failRegexIndex, failRegex in enumerate(self.__failRegex):
            try:
                ...
                if (
                    self.__ignoreRegex
                    and self._ignoreLine(buf, orgBuffer, failRegex) is not None
                ):
                ...
                if noDate:
                    self._logWarnOnce(
                        "_next_noTimeWarn",
                        (
                            "Found a match but no valid date/time found for %r.",
                            tupleLine[1],
                        ),
                        (
                ...
                if mlfid is not None:
                    ...
                    if fail.get("nofail"):
                        if ll <= 7:
                            logSys.log(
                                7,
                                "Nofail by mlfid %r in regex %s: %s",
                                mlfid,
                                failRegexIndex,
                    ...
                    # matched lines:
                    fail["matches"] = (
                        fail.get("matches", []) + failRegex.getMatchedTupleLines()
                    )
                # failure-id:
                fid = fail.get("fid")
                # ip-address or host:
                ...
                if host is None:
                    if ll <= 7:
                        logSys.log(
                            7,
                            "No failure-id by mlfid %r in regex %s: %s",
                            mlfid,
                            failRegexIndex,
                            fail.get("mlfforget", "waiting for identifier"),
                        )
                    fail["mlfpending"] = 1
                    # mark failure is pending
    ...
    def status(self, flavor="basic"):
        ...
        ret = [
            ("Currently failed", self.failManager.size()),
...
class FileFilter(Filter):
    ...
    def getFailures(self, filename, inOperation=None):
        ...
        try:
            ...
            # see http://python.org/dev/peps/pep-3151/
                ...
                logSys.error(
                    "Internal error in FileContainer open method - please report as a bug to https://github.com/fail2ban/fail2ban/issues"
                )
                logSys.exception(e)
                return False
            ...
            if self.__autoSeek:
                ...
                if startTime:
                    ...
                    try:
                        ...
                        logSys.error(
                            'Error during seek to start time in "%s"', filename
                        )
                        raise
                        logSys.exception(e)
                        return False
            ...
            if has_content:
                while not self.idle:
                    ...
                    self.inOperation = (
                        inOperation if inOperation is not None else log.inOperation
                    )
    ...
    def seekToTime(self, container, date, accuracy=3):
        ...
        if logSys.getEffectiveLevel() <= logging.DEBUG:
            logSys.debug(
                "Seek to find time %s (%s), file size %s",
                date,
                MyTime.time2str(date),
                fs,
            )
            ...
            while True:
                ...
                if timeMatch:
                    dateTimeMatch = self.dateDetector.getTime(
                        line[timeMatch.start() : timeMatch.end()], (timeMatch, template)
                    )
                else:
        ...
        if logSys.getEffectiveLevel() <= logging.DEBUG:
            logSys.debug(
                ...
                fs,
                foundTime,
                (MyTime.time2str(foundTime) if foundTime is not None else ""),
                cntr,
            )
...
class FileContainer:
    ...
    def decode_line(filename, enc, line):
        try:
            ...
            logSys.log(lev, "Error decoding line from '%s' with '%s'.", filename, enc)
