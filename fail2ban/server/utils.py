...
signame = dict(
    (num, name) for name, num in signal.__dict__.iteritems() if name.startswith("SIG")
)
...
class Utils:
    ...
    def executeCmd(
        ...
        try:
            ...
            if retcode is None:
                ...
                retcode = Utils.wait_for(
                    _popen_wait_end, timeout, Utils.DEFAULT_SHORTEST_INTERVAL
                )
                if retcode:
                    retcode = retcode[1]
            # if timeout:
        ...
        if output or std_level >= logSys.getEffectiveLevel():
            ...
            if popen.stdout:
                ...
                if (
                    stdout is not None
                    and stdout != ""
                    and std_level >= logSys.getEffectiveLevel()
                ):
                    for l in stdout.splitlines():
                        logSys.log(
                            std_level, "%x -- stdout: %r", realCmdId, uni_decode(l)
                        )
            if popen.stderr:
                try:
                    if retcode is None or retcode < 0:
                ...
                if (
                    ...
                    for l in stderr.splitlines():
                        logSys.log(
                            std_level, "%x -- stderr: %r", realCmdId, uni_decode(l)
                        )
        ...
        if retcode in success_codes:
            ...
            logSys.error(
                ...
                realCmdId,
                signame.get(sigcode, "signal %i" % sigcode),
                retcode,
            )
        else:
    ...
    def load_python_module(pythonModule):
        pythonModuleName = os.path.splitext(os.path.basename(pythonModule))[0]
        if sys.version_info >= (3, 3):
            mod = importlib.machinery.SourceFileLoader(
                pythonModuleName, pythonModule
            ).load_module()
        else:
            mod = imp.load_source(pythonModuleName, pythonModule)
        return mod
