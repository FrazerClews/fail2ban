    ...
    def found_terminator(self):
        try:
            ...
            try:
                ...
                logSys.error(
                    "PROTO-error: load message failed: %s",
                    e,
            ...
            if not isinstance(
                e, RequestHandler.LoadError
            ):  # pragma: no cover - normally unreachable
                logSys.error(
                    "Caught unhandled exception: %r",
                    e,
                    exc_info=logSys.getEffectiveLevel() <= logging.DEBUG,
                )
            # Sends the response to the client.
...
def loop(active, timeout=None, use_poll=False, err_count=None):
    ...
    if callable(use_poll):
        ...
        use_poll and asyncore.poll2 and hasattr(asyncore.select, "poll")
    ):  # pragma: no cover
        ...
        try:
            ...
            if err_count["listen"] < 20:
                ...
                if (
                    ...
                    logSys.critical(
                        "Too many errors - critical count reached %r", err_count
                    )
                    break
    ...
    def handle_accept(self):
        try:
            ...
            if self.__errCount["accept"] < 20:
                ...
                if (
                    isinstance(e, socket.error) and e.args[0] == errno.EMFILE
                ) or sum(  # [Errno 24] Too many open files
                    self.__errCount.itervalues()
                ) > 1000:
                    logSys.critical(
                        "Too many errors - critical count reached %r", self.__errCount
                    )
                    self.stop()
            return
