...
class MyTime:
    ...
    def time2str(unixTime, format="%Y-%m-%d %H:%M:%S"):
        ...
        dt = (
            datetime.datetime.fromtimestamp(unixTime).replace(microsecond=0)
            if unixTime < 253402214400
            else datetime.datetime(9999, 12, 31, 23, 59, 59)
        )
    ...
    _str2sec_parts = (
        ...
        (
            re.compile(_str2sec_subpart % r"weeks?|wee?|ww?"),
            "*" + str(7 * 24 * 60 * 60),
        ),
        (
            re.compile(_str2sec_subpart % r"months?|mon?"),
            "*" + str((365 * 3 + 366) * 24 * 60 * 60 / 4 / 12),
        ),
        (
            re.compile(_str2sec_subpart % r"years?|yea?|yy?"),
            "*" + str((365 * 3 + 366) * 24 * 60 * 60 / 4),
        ),
