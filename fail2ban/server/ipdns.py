...
__author__ = (
    "Fail2Ban Developers, Alexander Koeppe, Serg G. Brester, Yaroslav Halchenko"
)
...
class DNSUtils:
    ...
    def dnsToIp(dns):
        ...
        saveerr = None
        for fam, ipfam in (
            (socket.AF_INET, IPAddr.FAM_IPv4),
            (socket.AF_INET6, IPAddr.FAM_IPv6),
        ):
            try:
        ...
        if not ips and saveerr:
            logSys.warning(
                "Unable to find a corresponding IP address for %s: %s", dns, saveerr
            )
...
class IPAddr(object):
    ...
    def __wrap_ipstr(ipstr):
        ...
        if len(s) > 2:
            raise ValueError(
                "invalid ipstr %r, too many plen representation" % (ipstr,)
            )
    ...
    def isSingle(self):
        ...
        return self._plen == {socket.AF_INET: 32, socket.AF_INET6: 128}.get(
            self._family, -1000
        )
