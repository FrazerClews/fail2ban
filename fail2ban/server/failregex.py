...
FCUSTNAME_CRE = re.compile(r"^(/?)F-([A-Z0-9_\-]+)$")
...
R_HOST[RI_ADDR] = r"\[?(?:%s|%s)\]?" % (
    R_HOST[RI_IPV4],
    R_HOST[RI_IPV6],
)
R_HOST[RI_HOST] = r"(?:%s|%s)" % (
    R_HOST[RI_ADDR],
    R_HOST[RI_DNS],
)
R_HOST[RI_SUBNET] = r"\[?(?:%s|%s)(?:/%s)?\]?" % (
    R_HOST[RI_IPV4],
    R_HOST[RI_IPV6],
    R_HOST[RI_CIDR],
)
...
class Regex:
    ...
    def __init__(self, regex, multiline=False, **kwargs):
        ...
        try:
            ...
            raise RegexException("Unable to compile regular expression '%s'" % regex)
        # set fetch handler depending on presence of alternate (or tuple) tags:
        self.getGroups = (
            self._getGroupsWithAlt
            if (self._altValues or self._tupleValues)
            else self._getGroups
        )
    ...
    def search(self, tupleLines, orgLines=None):
        ...
        if self._matchCache:
            ...
            if len(orgLines) <= 1:
                ...
                try:
                    matchLineStart = (
                        self._matchCache.string.rindex(
                            "\n", 0, self._matchCache.start() + 1
                        )
                        + 1
                    )
                except ValueError:
                ...
                try:
                    matchLineEnd = (
                        self._matchCache.string.index("\n", self._matchCache.end() - 1)
                        + 1
                    )
                except ValueError:
    ...
    def getSkippedLines(self):
        ...
        while True:
            ...
            # KeyError is because of PyPy issue1665 affecting pypy <= 2.2.1
                ...
                if (
                    "PyPy" not in sys.version
                ):  # pragma: no cover - not sure this is even reachable
                    raise
                break
...
class FailRegex(Regex):
    ...
    def __init__(self, regex, prefRegex=None, **kwargs):
        ...
        if not [
            grp for grp in FAILURE_ID_PRESENTS if grp in self._regexObj.groupindex
        ] and (
            prefRegex is None
            or not [
                grp
                for grp in FAILURE_ID_PRESENTS
                if grp in prefRegex._regexObj.groupindex
            ]
        ):
    ...
    def getIP(self):
        fail = self.getGroups()
        return IPAddr(
            self.getFailID(("ip4", "ip6")), int(fail.get("cidr") or IPAddr.CIDR_UNSPEC)
        )
