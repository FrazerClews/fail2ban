...
def zone2offset(tz, dt):
    ...
    if tzo[3] != ":":
        # [+-]hhmm --> [+-]1 * (hh*60 + mm)
        return TZ_ABBR_OFFS[tz] + (-1 if tzo[0] == "-" else 1) * (
            int(tzo[1:3]) * 60 + int(tzo[3:5])
        )
    else:
        # [+-]hh:mm --> [+-]1 * (hh*60 + mm)
        return TZ_ABBR_OFFS[tz] + (-1 if tzo[0] == "-" else 1) * (
            int(tzo[1:3]) * 60 + int(tzo[4:6])
        )
...
def reGroupDictStrptime(found_dict, msec=False, default_tz=None):
    ...
    now = (
        year
    ) = month = day = hour = minute = tzoffset = weekday = julian = week_of_year = None
    second = fraction = 0
    ...
    if month is None or day is None:
        ...
        if julian is None and week_of_year is not None and weekday is not None:
            julian = _calc_julian_from_U_or_W(
                year, week_of_year, weekday, (week_of_year_start == 0)
            )
        # Cannot pre-calculate datetime.datetime() since can change in Julian
        # calculation and thus could have different value for the day of the week
        # calculation.
        if julian is not None:
            datetime_result = datetime.datetime.fromordinal(
                (julian - 1) + datetime.datetime(year, 1, 1).toordinal()
            )
    ...
    if assume_year:
        if not now:
            now = MyTime.now()
        if date_result > now + datetime.timedelta(
            days=1
        ):  # ignore by timezone issues (+24h)
