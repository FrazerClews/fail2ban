...
FCUSTAG_CRE = re.compile(r"<F-([A-Z0-9_\-]+)>")
# currently uppercase only
...
class CallingMap(MutableMapping, object):
    ...
    def __getitem__(self, key):
        ...
        if callable(value):
            ...
            value = (
                value(self)
                if hasattr(value, "__code__") and value.__code__.co_argcount
                else value()
            )
...
class CommandAction(ActionBase):
    ...
    def _operationExecuted(self, tag, family, *args):
        ...
        if not len(args):  # get
            ...
            return [
                v for f, v in self.__substCache.get(key, {}).iteritems() if family(f)
            ]
        cmd = args[0]
        if cmd:  # set:
            try:
    ...
    def _executeOperation(self, tag, operation, family=[], afterExec=None):
        ...
        for famoper in family:
            try:
                ...
                if cmd and cmd not in self._operationExecuted(
                    tag, lambda f: f != famoper
                ):
        ...
        if not res:
            raise RuntimeError(
                "Error %s action %s/%s: %r" % (operation, self._jail, self._name, err)
            )
        return res
    ...
    def _start(self, family=None, forceStart=False):
        ...
        if self._startOnDemand:
            if not forceStart:
                return True
        elif not forceStart and self.__started.get(
            family
        ):  # pragma: no cover - normally unreachable
            return True
        ...
        ret = self._executeOperation(
            "<actionstart>", "starting", family=family, afterExec=_started
        )
        return ret
    ...
    def ban(self, aInfo, cmd="<actionban>"):
        ...
        self.__started[family] = self.__started.get(family, 0) | 3
        # started and contains items

    @property
    def _prolongable(self):
        return (
            hasattr(self, "actionprolong")
            and self.actionprolong
    ...
    def flush(self):
        ...
        family = [f for (f, v) in self.__started.iteritems() if v & 3 == 3]
        # started and contains items
        ...
        def _afterFlush(family, ret):
            if ret and self.__started.get(family):
                self.__started[family] &= ~2
                # no items anymore

        return self._executeOperation(
            "<actionflush>", "flushing", family=family, afterExec=_afterFlush
        )
    ...
    def _stop(self, family=None):
        ...
        return self._executeOperation(
            "<actionstop>", "stopping", family=family, afterExec=_stopped
        )
    ...
    def replaceTag(cls, query, aInfo, conditional="", addrepl=None, cache=None):
        ...
        if not noRecRepl:
            ...
            if subInfo is None:
                subInfo = substituteRecursiveTags(
                    aInfo, conditional, ignore=cls._escapedTags, addrepl=addrepl
                )
            # cache if possible:
    ...
    def invalidateBanEpoch(self):
        ...
        if self._jail is not None:
            self._banEpoch = self._jail.actions.banEpoch = (
                self._jail.actions.banEpoch + 1
            )
        else:
    ...
    def _processCmd(self, cmd, aInfo=None):
        ...
        while True:
            ...
            if repcnt and self.actioncheck:
                ...
                def _beforeRepair():
                    if cmd == "<actionunban>" and not self._properties.get(
                ...
                ret = self._invariantCheck(
                    family, _beforeRepair, forceStart=(cmd != "<actionunban>")
                )
    ...
    def executeCmd(realCmd, timeout=60, **kwargs):
        ...
        with _cmd_lock:
            return Utils.executeCmd(
                realCmd, timeout, shell=True, output=False, **kwargs
            )
