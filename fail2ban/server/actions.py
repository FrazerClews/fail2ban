...
class Actions(JailThread, Mapping):
    ...
    def _load_python_module(pythonModule):
        ...
        if not hasattr(mod, "Action"):  # pragma: no cover
            raise RuntimeError("%s module does not have 'Action' class" % pythonModule)
    ...
    def stopActions(self, actions=None):
        ...
        for name, action in revactions:
            try:
                ...
                logSys.error(
                    "Failed to stop jail '%s' action '%s': %s",
                    self._jail.name,
                    name,
                    e,
    ...
    def run(self):
        ...
        for name, action in self._actions.iteritems():
            try:
                ...
                logSys.error(
                    "Failed to start jail '%s' action '%s': %s",
                    self._jail.name,
                    name,
                    e,
                    exc_info=logSys.getEffectiveLevel() <= logging.DEBUG,
        ...
        while self.active:
            ...
            bancnt = 0
            if Utils.wait_for(
                lambda: not self.active or self._jail.hasFailTickets, self.sleeptime
            ):
            ...
            if not bancnt or cnt >= self.banPrecedence:
                if self.active:
                    ...
                    self.__checkUnBan(
                        bancnt
                        if bancnt and bancnt < self.unbanMaxCount
                        else self.unbanMaxCount
                    )
                cnt = 0
    ...
    class ActionInfo(CallingMap):
        ...
        def copy(self):  # pragma: no cover
            return self.__class__(
                self.__ticket, self.__jail, self.immutable, self.data.copy()
            )
    ...
    def __checkBan(self, tickets=None):
        ...
        for ticket in tickets:
            ...
            if self.__banManager.addBanTicket(bTicket, reason=reason):
                ...
                logSys.notice(
                    "[%s] %sBan %s",
                    self._jail.name,
                    ("" if not bTicket.restored else "Restore "),
                    ip,
                )
                # do actions :
                for name, action in self._actions.iteritems():
                ...
                if (
                    self.banEpoch
                ):  # be sure tickets always have the same ban epoch (default 0):
                ...
                if bTicket.banned:
                    ...
                    if bTicket.banEpoch == self.banEpoch and diftm > 3:
                        ...
                        if (
                            not rebanacts
                            and MyTime.time() > self.__lastConsistencyCheckTM + 3
                        ):
        ...
        if cnt:
            logSys.debug(
                "Banned %s / %s, %s ticket(s) in %r",
                cnt,
                self.__banManager.getBanTotal(),
                self.__banManager.size(),
                self._jail.name,
            )
        return cnt
    ...
    def __reBan(self, ticket, actions=None, log=True):
        ...
        if log:
            logSys.notice(
                ...
                self._jail.name,
                aInfo["ip"],
                (", action %r" % actions.keys()[0] if len(actions) == 1 else ""),
            )
        for name, action in actions.iteritems():
            try:
                ...
                logSys.error(
                    "Failed to execute reban jail '%s' action '%s' " "info '%r': %s",
                    self._jail.name,
                    name,
                    aInfo,
                    e,
    ...
    def _prolongBan(self, ticket):
        ...
        for name, action in self._actions.iteritems():
            try:
                ...
                logSys.error(
                    "Failed to execute ban jail '%s' action '%s' " "info '%r': %s",
                    self._jail.name,
                    name,
                    aInfo,
                    e,
    ...
    def __flushBan(self, db=False, actions=None, stop=False):
        ...
        unbactions = {}
        for name, action in (
            actions if actions is not None else self._actions
        ).iteritems():
            try:
                if hasattr(action, "flush") and (
                    not isinstance(action, CommandAction) or action.actionflush
                ):
                ...
                logSys.error(
                    "Failed to flush bans in jail '%s' action '%s': %s",
                    self._jail.name,
                    name,
                    e,
                    exc_info=logSys.getEffectiveLevel() <= logging.DEBUG,
                ...
                if hasattr(action, "consistencyCheck"):
                    ...
                    def _beforeRepair():
                        if stop and not getattr(
                            action, "actionrepair_on_unban", None
                        ):  # don't need repair on stop
                            self._logSys.error(
                                "Invariant check failed. Flush is impossible."
                            )
                            return False
                        return True
    ...
    def __unBan(self, ticket, actions=None, log=True):
        ...
        for name, action in unbactions.iteritems():
            try:
                ...
                logSys.error(
                    "Failed to execute unban jail '%s' action '%s' " "info '%r': %s",
                    self._jail.name,
                    name,
                    aInfo,
                    e,
    ...
    def status(self, flavor="basic"):
        ...
        if flavor is None or flavor not in supported_flavors:
            logSys.warning(
                "Unsupported extended jail status flavor %r. Supported: %s"
        ...
        if flavor == "cymru":
            ...
            ret += [
                ...
                (
                    "Banned Country list",
                    self.__banManager.geBanListExtendedCountry(cymru_info),
                ),
