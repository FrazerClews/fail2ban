...
__copyright__ = (
    "Copyright (c) 2004 Cyril Jaquier, 2011-2012 Lee Clemens, 2012 Yaroslav Halchenko"
)
__license__ = "GPL"
...
class Jail(object):
    ...
    def __init__(self, name, backend="auto", db=None):
        ...
        if len(name) >= 26:
            logSys.warning(
                "Jail name %r might be too long and some commands "
                "might not function correctly. Please shorten" % name
            )
        self.__name = name
    ...
    def _setBackend(self, backend):
        ...
        if backend != "auto":
            ...
            if not (backend in self._BACKENDS):
                logSys.error(
                    "Unknown backend %s. Must be among %s or 'auto'"
                    % (backend, backends)
                )
                raise ValueError(
                    "Unknown backend %s. Must be among %s or 'auto'"
        ...
        for b in backends:
            ...
            try:
                ...
                if backend != "auto" and b != backend:
                    logSys.warning(
                        "Could only initiated %r backend whenever "
    ...
    def setBanTimeExtra(self, opt, value):
        ...
        if (
            opt in ["formula", "factor", "maxtime", "rndtime", "multipliers"]
            or be.get("evformula", None) is None
        ):
            ...
            if opt == "multipliers":
                be["evmultipliers"] = [
                    int(i)
                    for i in (
                        value.split(" ") if value is not None and value != "" else []
                    )
                ]
            ...
            if len(multipliers):
                evformula = lambda ban, banFactor=banFactor: (
                    ban.Time
                    * banFactor
                    * multipliers[ban.Count if ban.Count < len(multipliers) else -1]
                )
            else:
                formula = be.get(
                    "formula",
                ...
                evformula = lambda ban, banFactor=banFactor, formula=formula: max(
                    ban.Time, eval(formula)
                )
            ...
            if not be.get("rndtime", None) is None:
                rndtime = be["rndtime"]
                evformula = lambda ban, evformula=evformula: (
                    evformula(ban) + random.random() * rndtime
                )
            # set to extra dict:
    ...
    def getMaxBanTime(self):
        ...
        return (
            self._banExtra.get("maxtime", -1)
            if self._banExtra.get("increment")
            else self.actions.getBanTime()
        )
    ...
    def restoreCurrentBans(self, correctBanTime=True):
        ...
        try:
            if self.database is not None:
                ...
                for ticket in self.database.getCurrentBans(
                    jail=self,
                    forbantime=forbantime,
                    ...
                    try:
                        ...
                        logSys.error(
                            "Restore ticket failed: %s",
                            e,
                            exc_info=logSys.getEffectiveLevel() <= logging.DEBUG,
                        )
        except Exception as e:  # pragma: no cover
            logSys.error(
                "Restore bans failed: %s",
                e,
                exc_info=logSys.getEffectiveLevel() <= logging.DEBUG,
            )
