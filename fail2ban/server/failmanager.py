...
class FailManager:
    ...
    def getFailCount(self):
        ...
        with self.__lock:
            return len(self.__failList), sum(
                [f.getRetry() for f in self.__failList.values()]
            )
    ...
    def cleanup(self, time):
        with self.__lock:
            todelete = [
                fid
                for fid, item in self.__failList.iteritems()
    ...
    def toBan(self, fid=None):
        with self.__lock:
            for fid in (
                [fid] if fid is not None and fid in self.__failList else self.__failList
            ):
