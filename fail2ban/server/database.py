...
if sys.version_info >= (3,):  # pragma: 2.x no cover
    ...
    def _json_dumps_safe(x):
        try:
            ...
            logSys.error(
                "json dumps failed: %r", e, exc_info=logSys.getEffectiveLevel() <= 4
            )
            x = "{}"
        return x
    ...
    def _json_loads_safe(x):
        try:
            ...
            logSys.error(
                "json loads failed: %r", e, exc_info=logSys.getEffectiveLevel() <= 4
            )
            x = {}
        return x
...
class Fail2BanDb(object):
    ...
    def _connectDB(self, checkIntegrity=False):
        ...
        try:
            self._db = sqlite3.connect(
                filename, check_same_thread=False, detect_types=sqlite3.PARSE_DECLTYPES
            )
            ...
            logSys.info("Connected to fail2ban persistent database '%s'", filename)
        ...
        try:
            ...
            logSys.warning("New database created. Version '%r'", self.createDb())
            ...
            if version < Fail2BanDb.__version__:
                ...
                if newversion == Fail2BanDb.__version__:
                    logSys.warning(
                        "Database updated from '%r' to '%r'", version, newversion
                    )
                else:  # pragma: no cover
                    logSys.error(
                        "Database update failed to achieve version '%r'"
                        ": updated from '%r' to '%r'",
    ...
    def _dbBackupFilename(self):
        try:
            ...
            self.__dbBackupFilename = (
                self._dbFilename + "." + time.strftime("%Y%m%d-%H%M%S", MyTime.gmtime())
            )
            return self.__dbBackupFilename
        ...
        try:
            ...
            Utils.executeCmd(
                (
                    ...
                    self._dbFilename,
                    self._dbBackupFilename,
                )
            )
            dbFileSize = os.stat(self._dbFilename).st_size
            if dbFileSize:
                logSys.info(
                    "  Repair seems to be successful, restored %d byte(s).", dbFileSize
                )
                # succeeded - try to reconnect:
                self._connectDB(checkIntegrity=True)
            else:
                logSys.info(
                    "  Repair seems to be failed, restored %d byte(s).", dbFileSize
                )
                raise RepairException("Recreate ...")
        except Exception as e:
            # if still failed, just recreate database as fallback:
            logSys.error(
                "  Error repairing of fail2ban database '%s': %s",
                self._dbFilename,
                e.args[0],
                exc_info=(
                    not isinstance(e, RepairException)
                    and logSys.getEffectiveLevel() <= 10
                ),
            )
    ...
    def updateDb(self, cur, version):
        ...
        try:
            logSys.info(
                "Upgrade database: %s from version '%r'",
                self._dbBackupFilename,
                version,
            )
            ...
            # if still failed, just recreate database as fallback:
            logSys.error(
                "Failed to upgrade database '%s': %s",
                self._dbFilename,
                e.args[0],
    ...
    def addJail(self, cur, jail):
        ...
        """
        cur.execute(
            "INSERT OR IGNORE INTO jails(name, enabled) VALUES(?, 1)", (jail.name,)
        )
        if cur.rowcount <= 0:
            cur.execute(
    ...
    def delJail(self, cur, jail):
        ...
        cur.execute("UPDATE jails SET enabled=0 WHERE name=?", (jail.name,))
    ...
    def getJailNames(self, cur, enabled=None):
        ...
        if enabled is None:
            ...
            cur.execute("SELECT name FROM jails WHERE enabled=%s" % (int(enabled),))
    ...
    def addLog(self, cur, jail, container):
        ...
        """
        return self._addLog(
            cur, jail, container.getFileName(), container.getPos(), container.getHash()
        )

    def _addLog(self, cur, jail, name, pos=0, md5=None):
        lastLinePos = None
        cur.execute(
            "SELECT firstlinemd5, lastfilepos FROM logs " "WHERE jail=? AND path=?",
            (jail.name, name),
        )
        try:
    ...
    def getJournalPos(self, cur, jail, name, time=0, iso=None):
        ...
        """
        return self._addLog(cur, jail, name, time, iso)
        # no hash, just time as iso
    ...
    def updateJournal(self, cur, jail, name, time, iso):
        ...
        name, time, iso :
                Journal name (typically systemd-journal) and last known time.
        """
        self._updateLog(cur, jail, name, time, iso)
        # no hash, just time as iso
    ...
    @commitandrollback
    def getBan(
        self, cur, ip, jail=None, forbantime=None, overalljails=None, fromtime=None
    ):
        ip = str(ip)
        if not overalljails:
    ...
    def _getCurrentBans(self, cur, jail=None, ip=None, forbantime=None, fromtime=None):
        ...
        if jail is not None:
            query = (
                "SELECT ip, timeofban, bantime, bancount, data FROM bips WHERE jail=?"
            )
            queryArgs.append(jail.name)
        else:
            query = (
                "SELECT ip, max(timeofban), bantime, bancount, data FROM bips WHERE 1"
            )
        if ip is not None:
            query += " AND ip=?"
            queryArgs.append(ip)
    ...
    def getCurrentBans(
        ...
        for ticket in self._getCurrentBans(
            cur, jail=jail, ip=ip, forbantime=forbantime, fromtime=fromtime
        ):
            ...
            try:
                ...
                if bantime == -2:  # todo: remove it in future version
                    bantime = (
                        jail.actions.getBanTime()
                        if jail is not None
                        else (correctBanTime if correctBanTime else 600)
                    )
                ...
                if bantime != -1 and timeofban + bantime <= fromtime:
                    ...
                    logSys.debug(
                        "ignore ticket (with new max ban-time %r): too old %r <= %r, ticket: %r",
                        bantime,
                        timeofban + bantime,
                        fromtime,
                        ticket,
    ...
    def _purge_bips(self, cur):
        ...
        cur.execute(
            ...
            (
                int(MyTime.time()) - self._purgeAge,
                self._outDatedFactor,
                int(MyTime.time()) - self._purgeAge,
            ),
        )
    ...
    def purge(self, cur):
        ...
        self._bansMergedCache = {}
        cur.execute(
            "DELETE FROM bans WHERE timeofban < ?", (MyTime.time() - self._purgeAge,)
        )
