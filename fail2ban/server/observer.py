...
class ObserverThread(JailThread):
    ...
    def stop(self, wtime=5, forceQuit=True):
        if self.active and self._notify:
            ...
            if self.wait_empty(wtime) or forceQuit:
                n.clear()
                self.active = False
                # leave outer (active) loop
                self._paused = True
                # leave inner (queue) loop
                self.__db = None
            else:
                self._notify = n
    ...
    def failureFound(self, failManager, jail, ticket):
        ...
        try:
            ...
            logSys.info(
                "[%s] Found %s, bad - %s, %s # -> %s%s",
                jail.name,
                ip,
                MyTime.time2str(unixTime),
                banCount,
                retryCount,
    ...
    def incrBanTime(self, jail, banTime, ticket):
        ...
        try:
            if banTime > 0 and be.get("increment", False):
                ...
                for banCount, timeOfBan, lastBanTime in jail.database.getBan(
                    ...
                    logSys.debug(
                        "IP %s was already banned: %s #, %s", ip, banCount, timeOfBan
                    )
                    ...
                    if ticket.getTime() > timeOfBan:
                        logSys.info(
                            ...
                            % (
                                ...
                                datetime.timedelta(seconds=int(orgBanTime)),
                                datetime.timedelta(seconds=int(banTime)),
                            )
                        )
                    else:
    ...
    def banFound(self, ticket, jail, btime):
        ...
        try:
            ...
            if btime != oldbtime:
                ...
                logSys.log(
                    5,
                    "[%s] Observer: prolong %s in %s",
                    jail.name,
                    ip,
                    (btime, oldbtime),
                )
                self.add_timer(
                    min(10, max(0, btime - oldbtime - 5)), self.prolongBan, ticket, jail
                )
