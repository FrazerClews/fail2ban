...
class Server:
    ...
    def addJail(self, name, backend):
        ...
        if self.__reload_state.get(name) and self.__jails.exists(name):
            ...
            if jail.backend == backend:
                ...
                logSys.info(
                    "Restart jail %r (reason: %r != %r)", name, jail.backend, backend
                )
    ...
    def reloadJails(self, name, opts, begin):
        if begin:
            # begin reload:
            if self.__reload_state and (
                name == "--all" or self.__reload_state.get(name)
            ):  # pragma: no cover
                raise ValueError("Reload already in progress")
            logSys.info(
                "Reload " + (("jail %s" % name) if name != "--all" else "all jails")
            )
            with self.__lock:
                # if single jail:
                if name != "--all":
    ...
    def addAction(self, name, value, *args):
        ...
        self.__jails[name].actions.add(value, *args, reload=name in self.__reload_state)
    ...
    def status(self):
        try:
            ...
            jails.sort()
            jailList = ", ".join(jails)
            ret = [("Number of jail", len(self.__jails)), ("Jail list", jailList)]
            return ret
        finally:
            self.__lock.release()
    ...
    def setLogLevel(self, value):
        ...
        with self.__loggingLock:
            ...
            getLogger("fail2ban").setLevel(
                ll
                if DEF_LOGTARGET != "INHERITED" or ll < logging.DEBUG
                else DEF_LOGLEVEL
            )
    ...
    def setLogTarget(self, target):
        ...
        with self.__loggingLock:
            ...
            if systarget == "SYSLOG":
                ...
                try:
                    facility = getattr(
                        logging.handlers.SysLogHandler, "LOG_" + facility
                    )
                except AttributeError:  # pragma: no cover
                    logSys.error(
                        "Unable to set facility %r, using 'DAEMON'",
                        logOptions.get("facility"),
                    )
                ...
                if (
                    self.__syslogSocket is not None
                    and os.path.exists(self.__syslogSocket)
                    and stat.S_ISSOCK(os.stat(self.__syslogSocket).st_mode)
                ):
            ...
            if logOptions.get("format", "") != "":
                ...
                verbose = 0
                if (
                    self.__verbose is not None and self.__verbose > 2
                ):  # pragma: no cover
                ...
                logSys.info(
                    ...
                    % (
                        (
                            target
                            if target != "SYSLOG"
                            else "%s (%s)" % (target, self.__syslogSocket)
                        ),
                        version.version,
                    )
                )
    ...
    def setSyslogSocket(self, syslogsocket):
        ...
        return self.__logTarget != "SYSLOG" or self.setLogTarget(self.__logTarget)
    ...
    def setDatabase(self, filename):
        ...
        if len(self.__jails) != 0:
            raise RuntimeError("Cannot change database when there are jails present")
