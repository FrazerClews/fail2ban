...
if (
    not hasattr(pyinotify, "__version__")
...
try:
    ...
    raise ImportError(
        "Pyinotify is probably not functional on this system: %s" % str(e)
    )
...
class FilterPyinotify(FileFilter):
    ...
    def callback(self, event, origin=""):
        ...
        if event.mask & (pyinotify.IN_CREATE | pyinotify.IN_MOVED_TO):
            ...
            pyinotify.IN_IGNORED | pyinotify.IN_MOVE_SELF | pyinotify.IN_DELETE_SELF
        ):
            assumeNoDir = event.mask & (
            ...
            if isWD and (assumeNoDir or not os.path.isdir(path)):
                ...
                for logpath in self.__watchDirs:
                    if logpath.startswith(path + pathsep) and (
                        assumeNoDir or not os.path.isdir(logpath)
                    ):
        ...
        if not isWF:
            logSys.debug(
                "Ignoring event (%s) of %s we do not monitor", event.maskname, path
            )
            return
    ...
    def _delWatch(self, wdInt):
        ...
        try:
            ...
            if m.get_path(wdInt) is not None and not str(e).endswith(
                "(EINVAL)"
            ):  # prama: no cover
    ...
    def _delDirWatcher(self, path_dir):
        ...
        try:
            ...
            if not self._delWatch(wdInt):  # pragma: no cover
                logSys.debug(
                    "Non-existing file watcher %r for directory %s", wdInt, path_dir
                )
    ...
    def run(self):
        ...
        self.__notifier = pyinotify.Notifier(
            self.__monitor, prcevent, timeout=self.__notify_maxtout
        )
        ...
        while self.active:
            try:
                ...
                def __check_events():
                    return not self.active or self.__notifier.check_events(
                        timeout=notify_maxtout
                    )
