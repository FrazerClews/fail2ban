...
class Ticket(object):
    ...
    def __eq__(self, other):
        try:
            return (
                self._ip == other._ip
                and round(self._time, 2) == round(other._time, 2)
    ...
    def getMatches(self):
        return [
            (line if not isinstance(line, (list, tuple)) else "".join(line))
    ...
    def adjustTime(self, time, maxTime):
        ...
        if time > self._time:
            ...
            if self._firstTime < time - maxTime:
                ...
                self._retry = int(
                    round(self._retry / float(time - self._firstTime) * maxTime)
                )
