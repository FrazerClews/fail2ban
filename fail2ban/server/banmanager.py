    ...
    def getBanListExtendedCymruInfo(self, timeout=10):
        ...
        try:
            for ip in banIPs:
                ...
                question = ip.getPTR(
                    "origin.asn.cymru.com" if ip.isIPv4 else "origin6.asn.cymru.com"
                )
                try:
                    ...
                    for rdata in answers:
                        asn, net, country, rir, changed = [
                            answer.strip("'\" ")
                            for answer in rdata.to_text().split("|")
                        ]
                ...
                ) as dnse:  # pragma: no cover
                    logSys.error(
                        "DNSException %r querying Cymru for %s TXT", dnse, question
                    )
                    ...
                    logSys.error(
                        "Unhandled Exception %r querying Cymru for %s TXT", e, question
                    )
                    if logSys.level <= logging.DEBUG:
                        logSys.exception(e)
                    return_dict["error"] = repr(e)
        ...
        with self.__lock:
            ...
            for fid, ticket in self.__banList.iteritems():
                ...
                if time > eob:
                    ...
                    if (
                        len(unBanList) >= maxCount
                    ):  # stop search cycle, so reset back the next check time
                        nextUnbanTime = self.__nextUnbanTime
                        break
