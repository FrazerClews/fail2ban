...
class DateDetectorCache(object):
    ...
    def templates(self):
        ...
        with self.__lock:
            if (
                self.__templates
            ):  # pragma: no cover - race-condition + multi-threaded environment only
                return self.__templates
    ...
    def _cacheTemplate(self, template):
        ...
        name = template.name
        if (
            not name.startswith("{^LN-BEG}")
            and not name.startswith("^")
            and hasattr(template, "regex")
        ):
...
class DateDetector(object):
    ...
    def appendTemplate(self, template):
        ...
        if isinstance(template, str):
            ...
            if not template:
                if key in ("{^LN-BEG}", "{DEFAULT}"):
                    flt = (
                        lambda template: template.flags & DateTemplate.LINE_BEGIN
                        if key == "{^LN-BEG}"
                        else None
                    )
        ...
        logSys.info(
            "  date pattern `%r`: `%s`", getattr(template, "pattern", ""), template.name
        )
        logSys.debug(
            "  date pattern regex for %r: %s",
    ...
    def addDefaultTemplate(self, filterTemplate=None, preMatch=None, allDefaults=True):
        ...
        for template in (
            DateDetector._defCache.templates
            if allDefaults
            else DateDetector._defCache.defaultTemplates
        ):
            # filter if specified:
            ...
            if preMatch is not None:
                ...
                template = _getAnchoredTemplate(
                    template,
                    wrap=lambda s: RE_DATE_PREMATCH.sub(
                        lambda m: DateTemplate.unboundPattern(s), preMatch
                    ),
                )
    ...
    def matchTime(self, line):
        ...
        log = (
            logSys.log if logSys.getEffectiveLevel() <= logLevel else lambda *args: None
        )
        ...
        if i < len(self.__templates):
            ...
            if template.flags & (DateTemplate.LINE_BEGIN | DateTemplate.LINE_END):
                ...
                log(
                    logLevel - 1,
                    "  try to match last template #%02i (from %r to %r): ...%r==%r %s %r==%r...",
                    i,
                    distance,
                    endpos,
                    line[distance - 1 : distance],
                    self.__lastPos[1],
                    line[distance:endpos],
                    line[endpos : endpos + 1],
                    self.__lastEndPos[2],
                )
                ...
                if (
                    ...
                    or (
                        line[distance] == self.__lastPos[2]
                        and not self.__lastPos[2].isalnum()
                    )
                ) and (
                    line[endpos : endpos + 1] == self.__lastEndPos[2]
                    or (
                        line[endpos - 1] == self.__lastEndPos[1]
                        and not self.__lastEndPos[1].isalnum()
                    )
                ):
                    # search in line part only:
                    log(
                        logLevel - 1,
                        "  boundaries are correct, search in part %r",
                        line[distance:endpos],
                    )
            ...
            if match:
                ...
                if (
                    ...
                    or template.flags  # single template:
                    & (DateTemplate.LINE_BEGIN | DateTemplate.LINE_END)
                    or (
                    ...
                    log(
                        logLevel,
                        "  ** last pattern collision - pattern change, reserve & search ...",
                    )
                    found = match, distance, endpos, i
                    # save current best alternative
                    match = None
            else:
                log(
                    logLevel, "  ** last pattern not found - pattern change, search ..."
                )
        ...
        if not match:
            ...
            for ddtempl in self.__templates:
                ...
                if match:
                    ...
                    log(
                        logLevel,
                        "  matched time template #%02i (at %r <= %r, %r) %s",
                        i,
                        distance,
                        ddtempl.distance,
                        self.__lastPos[0],
                    ...
                    if (distance == 0 and ddtempl.hits) and not self.__templates[
                        i + 1
                    ].template.hits:
                        break
                    ...
                    if distance > ddtempl.distance or distance > self.__lastPos[0]:
                        log(
                            logLevel,
                            "  ** distance collision - pattern change, reserve",
                        )
                        ## shortest of both:
                        if distance < found[1]:
                            found = match, distance, endpos, i
        ...
        if template is not None:
            try:
                date = template.getDate(
                    line, timeMatch[0], default_tz=self.__default_tz
                )
                if date is not None:
                    if (
                        logSys.getEffectiveLevel() <= logLevel
                    ):  # pragma: no cover - heavy debug
                        logSys.log(
                            logLevel,
                            "  got time %f for %r using template %s",
                            date[0],
                            date[1].group(1),
                            template.name,
                        )
    ...
    def _reorderTemplate(self, num):
        ...
        if num:
            ...
            if logSys.getEffectiveLevel() <= logLevel:
                logSys.log(
                    logLevel, "  -> reorder template #%02i, hits: %r", num, ddtempl.hits
                )
            ...
            def _moveable():
                ...
                if logSys.getEffectiveLevel() <= logLevel:
                    logSys.log(
                        logLevel,
                        "  -> compare template #%02i & #%02i, weight %.3f > %.3f, hits %r > %r",
                        num,
                        pos,
                        weight,
                        pweight,
            ...
            while (
                self.__firstUnused < len(templates)
                and templates[self.__firstUnused].hits
            ):
                self.__firstUnused += 1
            if logSys.getEffectiveLevel() <= logLevel:
