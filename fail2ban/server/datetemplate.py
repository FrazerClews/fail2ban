...
class DateTemplate(object):
    ...
    def setRegex(self, regex, wordBegin=True, wordEnd=True):
        ...
        if boundEnd:
            ...
            regex += r"(?=\b|\W|$)"
        if not (self.flags & DateTemplate.LINE_BEGIN) and RE_LINE_BOUND_BEG.search(
            regex
        ):
    ...
    regex = property(
        getRegex,
        setRegex,
        doc="""Regex used to search for date.
		""",
    )
...
class DateEpoch(DateTemplate):
    ...
    def __init__(self, lineBeginOnly=False, pattern=None, longFrm=False):
        ...
        if pattern:
            ...
            regex = (
                "(" + RE_EPOCH_PATTERN.sub(lambda v: "(%s)" % epochRE, pattern) + ")"
            )
            self._grpIdx = 2
            self.setRegex(regex)
        elif not lineBeginOnly:
            regex = (
                r"((?:^|(?P<square>(?<=^\[))|(?P<selinux>(?<=\baudit\()))%s)(?:(?(selinux)(?=:\d+\)))|(?(square)(?=\])))"
                % epochRE
            )
            self.setRegex(regex, wordBegin=False)
            # already line begin resp. word begin anchored
        else:
...
class DatePatternRegex(DateTemplate):
    ...
    def setRegex(self, pattern, wordBegin=True, wordEnd=True):
        ...
        try:
            ...
            raise TypeError(
                "Failed to set datepattern '%s' (may be an invalid format or unescaped percent char): %s"
                % (pattern, e)
            )
    ...
    def getDate(self, line, dateMatch=None, default_tz=None):
        ...
        if dateMatch:
            return (
                reGroupDictStrptime(dateMatch.groupdict(), default_tz=default_tz),
                dateMatch,
            )
