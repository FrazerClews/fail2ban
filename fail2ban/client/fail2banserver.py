...
class Fail2banServer(Fail2banCmdLine):
    ...
    def startServerDirect(conf, daemon=True):
        logSys.debug(
            "  direct starting of server in %s, deamon: %s", os.getpid(), daemon
        )
    ...
    def startServerAsync(conf):
        ...
        if frk:  # pragma: no cover
            pid = os.fork()
        logSys.debug(
            "  async starting of server in %s, fork: %s - %s", os.getpid(), frk, pid
        )
        if pid == 0:
            args = list()
            args.append(SERVER)
            ...
            try:
                ...
                if frk:  # pragma: no cover
                    ...
                    if ret != 0:  # pragma: no cover
                        raise OSError(
                            ret,
                            "Unknown error by executing server %r with %r"
                            % (args[1], exe),
                        )
                ...
                # Use the PATH env.
                logSys.warning(
                    "Initial start attempt failed (%s). Starting %r with the same args",
                    e,
                    SERVER,
                )
                if frk:  # pragma: no cover
    ...
    def getServerPath():
        ...
        if not os.path.isfile(
            exe
        ):  # may be unresolved in test-cases, so get relative starter (client):
            startdir = os.path.dirname(sys.argv[0])
            exe = os.path.abspath(os.path.join(startdir, SERVER))
            if not os.path.isfile(
                exe
            ):  # may be unresolved in test-cases, so try to get relative bin-directory:
                startdir = os.path.dirname(os.path.abspath(__file__))
                startdir = os.path.join(
                    os.path.dirname(os.path.dirname(startdir)), "bin"
                )
                exe = os.path.abspath(os.path.join(startdir, SERVER))
        return exe
    ...
    def start(self, argv):
        ...
        try:
            ...
            if (
                len(args) == 1
                and args[0] == "start"
                and not self._conf.get("interactive", False)
            ):
                pass
            else:
            ...
            if not nonsync:
                ...
                Utils.wait_for(
                    lambda: phase.get("ready", None) is not None,
                    self._conf["timeout"],
                    0.001,
                )
            ...
            if not nonsync and cli:
                Utils.wait_for(
                    lambda: phase.get("done", None) is not None,
                    self._conf["timeout"],
                    0.001,
                )
