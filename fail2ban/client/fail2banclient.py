...
class Fail2banClient(Fail2banCmdLine, Thread):
    ...
    def dispInteractive(self):
        output(
            "Fail2Ban v"
            + version
            + " reads log file that contains password failure report"
        )
    ...
    def __ping(self, timeout=0.1):
        return self.__processCmd(
            [["ping"] + ([timeout] if timeout != -1 else [])], False, timeout=timeout
        )
            ...
            if os.access(self._conf["socket"], os.F_OK):  # pragma: no cover
                ...
                if os.access(self._conf["socket"], os.W_OK):
                    ...
                    if errorOnly:
                        ...
                        logSys.error(
                            "%sUnable to contact server. Is it running?",
                            ("[%s] " % prevError) if prevError else "",
                        )
                else:
                    logSys.error(
                        "Permission denied to socket: %s," " (you must be root)",
                        self._conf["socket"],
                    )
            else:
                logSys.error(
                    "Failed to access socket path: %s." " Is fail2ban running?",
                    self._conf["socket"],
                )
        except Exception as e:  # pragma: no cover
            logSys.error(
                "Exception while checking socket access: %s", self._conf["socket"]
            )
            logSys.error(e)
    ...
    def __startServer(self, background=True):
        ...
        try:
            if background:
                ...
                th = Thread(
                    target=Fail2banClient.__processStartStreamAfterWait,
                    args=(self, stream, False),
                )
                th.daemon = True
                th.start()
    ...
    def configureServer(self, nonsync=True, phase=None):
        ...
        if nonsync:
            th = Thread(
                target=Fail2banClient.configureServer, args=(self, False, phase)
            )
            th.daemon = True
            return th.start()
        ...
        if phase is not None:
            Utils.wait_for(
                lambda: phase.get("start-ready", None) is not None, 0.5, 0.001
            )
    ...
    def __processStartStreamAfterWait(self, *args):
        try:
            ...
            if self._conf["verbose"] > 1:
                logSys.exception(e)
            logSys.error(
                "Could not start server. Maybe an old "
    ...
    def heartbeat(self):
        ...
        if not self.pos:
            sys.stdout.write(
                "\nINFO   [#"
                + (" " * self.maxpos)
                + "] Waiting on the server...\r\x1b[8C"
            )
        self.pos += self.delta
        if self.delta > 0:
