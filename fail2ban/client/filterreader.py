...
class FilterReader(DefinitionInitConfigReader):
    ...
    def applyAutoOptions(self, backend):
        ...
        if not self._initOpts.get("logtype") and not self.has_option(
            "Definition", "logtype", False
        ):
            self._initOpts["logtype"] = ["file", "journal"][
                int(backend.startswith("systemd"))
            ]
    ...
    def _fillStream(stream, opts, jailName):
        ...
        for opt, value in opts.iteritems():
            if opt in ("failregex", "ignoreregex"):
                ...
                stream.insert(
                    0 if opt == "usedns" else prio0idx, ["set", jailName, opt, value]
                )
                prio0idx += 1
