...
class JailReader(ConfigReader):
    ...
    def _glob(path):
        ...
        for p in glob.glob(path):
            if os.path.exists(p):
                ...
                logSys.warning(
                    "File %s is a dangling link, thus cannot be monitored" % p
                )
        return pathList
    ...
    def getOptions(self):
        ...
        self.merge_defaults({"fail2ban_version": version})

        try:

            # Read first options only needed for merge defaults ('known/...' from filter):
            self.__opts = ConfigReader.getOptions(
                self, self.__name, self._configOpts1st, shouldExist=True
            )
            ...
            if flt:
                ...
                ConfigReader.merge_section(
                    self, self.__name, self.__filter.getCombined(), "known/"
                )
            else:
                self.__filter = None
                    ...
                    if actName.endswith(".py"):
                        self.__actions.append(
                            [
                                ...
                                os.path.join(self.getBaseDir(), "action.d", actName),
                                json.dumps(actOpt),
                            ]
                        )
                    else:
        ...
        if e:
            stream.extend(
                [
                    [
                        "config-error",
                        "Jail '%s' skipped, because of wrong configuration: %s"
                        % (self.__name, e),
                    ]
                ]
            )
