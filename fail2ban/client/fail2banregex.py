...
class LineStats(object):
    ...
    def __str__(self):
        return (
            "%(tested)d lines, %(ignored)d ignored, %(matched)d matched, %(missed)d missed"
            % self
        )
...
class Fail2banRegex(object):
    ...
    def setDatePattern(self, pattern):
        if not self._datepattern_set:
            ...
            if pattern is not None:
                self.output(
                    "Use      datepattern : %s" % (self._filter.getDatePattern()[1],)
                )
    ...
    def _dumpRealOptions(self, reader, fltOpt):
        ...
        for k in ["logtype", "datepattern"] + fltOpt.keys():
            ...
            try:
                realopts[k] = (
                    combopts[k] if k in combopts else reader.get("Definition", k)
                )
    ...
    def readRegex(self, value, regextype):
        ...
        if fltFile is not None:
            if (
                ...
                self.output(
                    "Use %11s filter file : %s, basedir: %s" % (regex, fltName, basedir)
                )
            else:
                ...
                if not os.path.isabs(
                    fltName
                ):  # avoid join with "filter.d" inside FilterReader
        ...
        for regextype, regex_values in regex_values.iteritems():
            ...
            for regex in regex_values:
                getattr(self._filter, "add%sRegex" % regextype.title())(
                    regex.getFailRegex()
                )
        return True
                ...
                for r in ret:
                    output(
                        ...
                        % (
                            r[1],
                            r[2],
                            dict((k, v) for k, v in r[3].iteritems() if k != "matches"),
                        )
                    )
        ...
        for line in test_lines:
            ...
            if is_ignored:
                ...
                if not self._print_no_ignored and (
                    self._print_all_ignored
                    or self._line_stats.ignored <= self._maxlines + 1
                ):
                    self._line_stats.ignored_lines.append(line)
                    if self._debuggex:
                        self._line_stats.ignored_lines_timeextracted.append(
                ...
                if self._print_all_matched:
                    ...
                    if self._debuggex:
                        self._line_stats.matched_lines_timeextracted.append(
                            line_datetimestripped
                        )
            else:
                self._line_stats.missed += 1
                if not self._print_no_missed and (
                    self._print_all_missed
                    or self._line_stats.missed <= self._maxlines + 1
                ):
                    self._line_stats.missed_lines.append(line)
                    if self._debuggex:
                        self._line_stats.missed_lines_timeextracted.append(
                            line_datetimestripped
                        )
        ...
        if lines:
            ...
            if self._debuggex:
                ...
                if lines < self._maxlines or getattr(self, "_print_all_" + ltype):
                    ...
                    output(
                        "%s too many to print.  Use --print-all-%s "
    ...
    def printStats(self):
        ...
        def print_failregexes(title, failregexes):
            ...
            for cnt, failregex in enumerate(failregexes):
                ...
                if match or self._verbose:
                    out.append(
                        "%2d) [%d] %s" % (cnt + 1, match, failregex.getFailRegex())
                    )
            ...
            for template in self._filter.dateDetector.templates:
                if self._verbose or template.hits:
                    ...
                    if self._verbose_date:
                        ...
                        out.append(
                            "    # regex:   %s" % (getattr(template, "regex", ""),)
                        )
        ...
        if self._time_elapsed is not None:
            output(
                "[processed in %.2f sec]" % self._time_elapsed,
            )
        output("")
    ...
    def start(self, args):
        ...
        try:
            if not self.readRegex(cmd_regex, "fail"):  # pragma: no cover
                return False
            if len(args) == 3 and not self.readRegex(
                args[2], "ignore"
            ):  # pragma: no cover
                return False
        ...
        if os.path.isfile(cmd_log):
            ...
            if self._filter.getMaxLines() <= 1 and "\n" not in cmd_log:
                self.output(
                    "Use      single line : %s" % shortstr(cmd_log.replace("\n", r"\n"))
                )
                test_lines = [cmd_log]
...
def exec_command_line(*args):
    ...
    if opts.print_no_missed and opts.print_all_missed:  # pragma: no cover
        errors.append(
            "ERROR: --print-no-missed and --print-all-missed are mutually exclusive."
        )
    if opts.print_no_ignored and opts.print_all_ignored:  # pragma: no cover
        errors.append(
            "ERROR: --print-no-ignored and --print-all-ignored are mutually exclusive."
        )
