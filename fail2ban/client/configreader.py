        ...
        if not self._cfg and self._cfg_share is not None:
            ...
            if not self._cfg:
                self._cfg = ConfigReaderUnshared(
                    share_config=self._cfg_share, **self._cfg_share_kwargs
                )
...
class ConfigReaderUnshared(SafeConfigParserWithIncludes):
    ...
    def read(self, filename):
        if not os.path.exists(self._basedir):
            raise ValueError(
                "Base configuration directory %s does not exist " % self._basedir
            )
        ...
        if len(config_files):
            ...
            if config_files_read:
                return True
            logSys.error(
                "Found no accessible config files for %r under %s",
                filename,
                self.getBaseDir(),
            )
            return False
            ...
            logSys.error(
                "Found no accessible config files for %r " % filename
                + (
                    [
                        "under %s" % self.getBaseDir(),
                        "among existing ones: " + ", ".join(config_files),
        ...
        for opttype, optname, optvalue in _OptionsTemplateGen(options):
            ...
            try:
                ...
                if not optvalue is None:
                    logSys.warning(
                        "'%s' not defined in '%s'. Using default one: %r"
...
class DefinitionInitConfigReader(ConfigReader):
        ...
        for opttype, optname, optvalue in _OptionsTemplateGen(configOpts):
            ...
            if conv:
                ...
                try:
                    ...
                    logSys.warning(
                        "Wrong %s value %r for %r. Using default one: %r",
                        opttype,
                        v,
                        optname,
                        optvalue,
    ...
    def getCombined(self, ignore=()):
        ...
        opts = substituteRecursiveTags(
            combinedopts, ignore=ignore, addrepl=self.getCombOption
        )
        if not opts:
