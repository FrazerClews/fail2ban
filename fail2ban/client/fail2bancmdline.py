...
class Fail2banCmdLine:
    ...
    def dispUsage(self):
        ...
        output(
            ...
            + caller
            + " [OPTIONS]"
            + (" <COMMAND>" if not caller.endswith("server") else "")
        )
        output("")
        output(
            "Fail2Ban v"
    ...
    def initCmdLine(self, argv):
        ...
        try:
            ...
            try:
                ...
                optList, self._args = getopt.getopt(
                    self._argv[1:], cmdOpts, cmdLongOpts
                )
            ...
            logSys.info(
                "Using pid file %s, [%s] logging to %s",
                self._conf["pidfile"],
                logging.getLevelName(llev),
                self._conf["logtarget"],
            )
    ...
    def readConfig(self, jail=None):
        ...
        try:
            ...
            ret = self.configurator.getOptions(
                jail, self._conf, ignoreWrong=not self.cleanConfOnly
            )
