...
class BadIPsAction(ActionBase):  # pragma: no cover - may be unavailable
    ...
    def isAvailable(timeout=1):
        try:
            response = urlopen(
                Request(
                    "/".join([BadIPsAction._badips]), headers={"User-Agent": "Fail2Ban"}
                ),
                timeout=timeout,
            )
            return True, ""
    ...
    def logError(
        self, response, what=""
    ):  # pragma: no cover - sporadical (502: Bad Gateway, etc)
        messages = {}
        try:
            ...
            pass
        self._logSys.error(
            "%s. badips.com response: '%s'", what, messages.get("err", "Unknown")
        )
    ...
    def getCategories(self, incParents=False):
        ...
        try:
            ...
            if not "categories" in response_json:
                err = (
                    "badips.com response lacked categories specification. Response was: %s"
                    % (response_json,)
                )
                self._logSys.error(err)
                raise ValueError(err)
            categories = response_json["categories"]
            categories_names = set(value["Name"] for value in categories)
            if incParents:
                categories_names.update(
                    set(value["Parent"] for value in categories if "Parent" in value)
                )
    ...
    def bancategory(self, bancategory):
        if bancategory != "any" and bancategory not in self.getCategories(
            incParents=True
        ):
            self._logSys.error(
                "Category name '%s' not valid. "
    ...
    def banaction(self, banaction):
        if banaction is not None and banaction not in self._jail.actions:
            self._logSys.error(
                "Action name '%s' not in jail '%s'", banaction, self._jail.name
            )
    ...
    def update(self):
        ...
        if self.banaction is not None:
            ...
            try:
                ips = self.getList(self.bancategory, self.score, self.age, self.bankey)
    ...
    def ban(self, aInfo):
        ...
        try:
            ...
            messages = json.loads(response.read().decode("utf-8"))
            self._logSys.debug("Response from badips.com report: '%s'", messages["suc"])
